    ...
    <div className="java-digest tech-blog-reader" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} style={{ alignItems: "center", background: "rgba(3,8,18,.72)", display: "flex", inset: 0, justifyContent: "center", padding: 16, position: "fixed", zIndex: 300 }}>
      <section role="dialog" aria-modal="true" aria-label={`${blog.title} full lesson`} style={{ background: "var(--jd-background)", border: `1px solid ${accent}55`, borderRadius: 12, boxShadow: "0 24px 80px rgba(0,0,0,.5)", color: "var(--jd-text)", display: "grid", gap: 14, gridTemplateRows: "auto minmax(0, 1fr)", maxHeight: "min(90vh, 900px)", maxWidth: 980, overflow: "hidden", width: "100%" }}>
        ...
        <div className="java-digest-reader-scroll" style={{ display: "grid", gap: 14, minWidth: 0, gridTemplateColumns: "minmax(0, 1fr)", minHeight: 0, overflowY: "auto", padding: "0 18px 20px" }}>
          ...
          <pre style={{ background: "var(--jd-surface-sunken)", border: "1px solid var(--jd-border)", borderRadius: 8, color: "var(--jd-code-text)", fontSize: 11, lineHeight: 1.5, margin: 0, overflowX: "auto", padding: 10, whiteSpace: "pre-wrap" }}>{blog.example}</pre>
          {blog.patterns?.length ? <section style={{ display: "grid", gap: 8 }}><strong style={{ color: accent, fontSize: 11.5 }}>Industry patterns</strong>{blog.patterns.map((pattern) => <article key={pattern.name} style={{ background: "var(--jd-surface-subtle)", border: "1px solid var(--jd-border)", borderRadius: 7, padding: "9px 10px" }}><strong style={{ fontSize: 11.5 }}>{pattern.name}</strong><CourseDiagram diagramKey={PATTERN_DIAGRAMS[pattern.name]} accent={accent} /><p style={{ color: "var(--jd-text-soft)", fontSize: 11.2, lineHeight: 1.48, margin: "4px 0 0" }}>{pattern.intent}</p><p style={{ color: "var(--jd-text-soft)", fontSize: 11.2, lineHeight: 1.48, margin: "3px 0 0" }}><b style={{ color: accent }}>Use:</b> {pattern.use} <b style={{ color: "var(--jd-warning)" }}>Watch:</b> {pattern.caution}</p></article>)}</section> : null}
