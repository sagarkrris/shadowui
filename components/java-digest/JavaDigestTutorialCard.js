...
export default function JavaDigestTutorialCard({ tutorial, accent, completedIds, bookmarkedIds, onToggleStatus, onOpen }) {
  return <article className="java-digest-card" style={{ ...wrap, borderRadius: 8, display: "grid", gap: 7, padding: 10 }}>
    ...
    <p style={{ fontSize: 11 }}>Reference context: {(tutorial.references || []).map(ref => <a key={ref.url} href={ref.url} target="_blank" rel="noreferrer">{ref.title}</a>)}</p>
    <details style={{ borderTop: "1px solid var(--jd-border)", paddingTop: 7 }}>
      ...
      <div style={{ display: "grid", gap: 7, marginTop: 8 }}>
        ...
        <p style={{ color: "var(--jd-text-soft)", fontSize: 11 }}>Contextual snippet: supply the enclosing method, imports and application types. Expected result describes behavior, not necessarily console output.</p><code style={{ background: "var(--jd-surface-sunken)", borderRadius: 6, color: "var(--jd-code-text)", display: "block", fontSize: 10.8, lineHeight: 1.45, padding: 7, whiteSpace: "pre-wrap" }}>{tutorial.example}</code>
