...
export default function HomeDemo({ onContinue, onSignIn, onOpenWorkspace }) {
  ...
  return (
    <section className="home-demo" aria-labelledby="home-demo-title">
      ...
      <div className="home-demo__footer">{onOpenWorkspace ? <button type="button" className="home-demo__java-digest-launch" onClick={() => onOpenWorkspace("javaDigest")}><i className="ti ti-news" /><span><strong>Open Java Digest</strong><small>Java + Spring interview curriculum</small></span><i className="ti ti-arrow-right" aria-hidden="true" /></button> : null}<button type="button" className="home-demo__primary" onClick={() => onContinue({})}>Build my personalized plan <span aria-hidden="true">→</span></button><button type="button" className="home-demo__text-button" onClick={() => onContinue({ skipped: true })}>Skip tour</button><span aria-hidden="true">·</span><button type="button" className="home-demo__text-button" onClick={onSignIn}>Already have an account? Sign in</button></div>
    </section>
  );
}
