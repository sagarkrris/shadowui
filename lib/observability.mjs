...
function emitMetric(payload) {
  const endpoint = process.env.METRICS_WEBHOOK_URL || process.env.ERROR_TRACKING_WEBHOOK_URL;
  if (!endpoint) return;
  fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json", ...(process.env.METRICS_WEBHOOK_TOKEN ? { Authorization: `Bearer ${process.env.METRICS_WEBHOOK_TOKEN}` } : process.env.ERROR_TRACKING_TOKEN ? { Authorization: `Bearer ${process.env.ERROR_TRACKING_TOKEN}` } : {}) }, body: JSON.stringify(payload) }).catch(() => undefined);
}
...
export function recordMetric(name, value = {}) {
  ...
  const metric = { type: "metric", name, count: counters.get(name), ...value, createdAt: new Date().toISOString() };
  emitMetric(metric);
  return metric;
}
