...
export function buildSpeechTranscript(results = []) {
  ...
  const finalText = joinSpeechSegments(finalSegments);
  const interimText = joinSpeechSegments(interimSegments);

  return {
    finalText,
    interimText,
    displayText: joinSpeechSegments([finalText, interimText]),
  };
}
