...
// Gradle accepts both Kotlin-style `implementation("g:a:v")` and the common
// Groovy-style `implementation 'g:a:v'` notation.
const GRADLE_DEPENDENCY = /(?:implementation|api|testImplementation|runtimeOnly)\s*(?:\(\s*)?['"]([^:'"]+):([^:'"]+)(?::([^'")]+))?['"]\s*\)?/g;
