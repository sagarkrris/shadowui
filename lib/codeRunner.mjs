...
export const CODE_RUNNER_FEATURE_STATE = {
  ...
  summary: "Live execution is paused while InterviewIQ moves to a configured sandbox runner.",
};
...
export function isCodeRunnerConfigured(url = PISTON_EXECUTE_URL) {
...
export function buildCodeRunnerError({ status = 502, body = {}, timedOut = false } = {}) {
  ...
  if (status === 503 && /not configured/i.test(String(body?.message || body?.error || ""))) {
    return {
      status: 503,
      error: "Live code execution is paused until PISTON_EXECUTE_URL is set to a self-hosted or approved Piston runner.",
      runnerUnavailable: true,
    };
  }
