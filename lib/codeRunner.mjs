...
export const CODE_RUNNER_FEATURE_STATE = {
  status: "ready",
  title: "Cloud sandbox ready",
  summary: "Run Java practice snippets through a configured sandbox runner, or keep using AI code review when execution is unavailable.",
};
...
export function buildCodeRunnerError({ status = 502, body = {}, timedOut = false } = {}) {
  ...
  if (status === 503 && /not configured/i.test(String(body?.message || body?.error || ""))) {
    return {
      status: 503,
      error: "Live code execution is paused until CODE_RUNNER_PROVIDER is set to vercel-sandbox or PISTON_EXECUTE_URL is set to a self-hosted or approved Piston runner.",
      runnerUnavailable: true,
    };
  }
