...
export function buildCompanyProviderStatus({ prep = null, refreshState = null, now = new Date() } = {}) {
  const activePrep = prep || getCompanyPrep();
  const refreshedAt = refreshState?.refreshedAt || activePrep.lastUpdated;
  const refreshedTime = Date.parse(refreshedAt);
  const nowTime = now.getTime();
  const ageDays = Number.isFinite(refreshedTime)
    ? Math.max(0, Math.floor((nowTime - refreshedTime) / 86400000))
    : null;

  return {
    company: activePrep.company,
    provider: activePrep.isSeeded ? "Seeded public reports" : "Generic public guide set",
    freshnessLabel: ageDays === null ? "Unknown freshness" : ageDays <= 7 ? "Fresh this week" : ageDays <= 30 ? "Fresh this month" : "Needs manual refresh",
    ageDays,
    verifiedCount: Object.keys(refreshState?.verifiedQuestions || {}).length,
    sourceCount: (refreshState?.sourceLinks || activePrep.resources || []).length,
    note: refreshState?.note || activePrep.caveat,
  };
}
...
export function deriveWeakSpots(messages = [], structuredSessions = []) {
  const text = messages
  ...
  const structuredGaps = (Array.isArray(structuredSessions) ? structuredSessions : [structuredSessions])
    .flatMap((session) => (session?.turns || []).flatMap((turn) => [...(turn.score?.gaps || []), ...(turn.score?.recommendations || [])]))
    .filter(Boolean);
  const structuredLabels = checks.filter(([label, pattern]) => structuredGaps.some((gap) => pattern.test(String(gap)))).map(([label]) => label);
  return Array.from(new Set([...structuredLabels, ...checks.filter(([, pattern]) => pattern.test(text)).map(([label]) => label)]));
}
