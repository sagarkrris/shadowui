...
import { buildMasteryMap, prioritizePracticeCards } from "./questionMemory.mjs";
...
const PRACTICE_PACKS = [
  ...
  },
  {
    ...
    stackKeys: ["node", "python", "go", "sap", "ruby", "rust"],
    aliases: ["backend", "api design", "rest", "authentication", "security", "microservices", "node", "python", "fastapi", "express", "django", "sap", "abap", "odata", "ruby", "rails", "rust", "tokio", "axum"],
    cards: [
      {
        id: "api-idempotency",
  ...
  },
  {
    ...
    icon: "ti-database",
    accent: "SQL + Data",
    stackKeys: ["sql", "postgresql", "mongodb"],
...
export function getPracticePack({
  ...
  const masteryMap = buildMasteryMap(questionMemory, bank, { now });
  const cards = prioritizePracticeCards(
    pickCards(bank, topic, { seed, excludeIds }),
    masteryMap,
  ).map((card) => ({
    ...card,
