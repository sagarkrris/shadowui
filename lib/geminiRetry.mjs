...
export function getSafeGeminiErrorMessage(error, fallback = "AI request failed. Please try again.") {
  const text = getErrorText(error);
  const code = String(error?.code || "");

  if (/InvalidParameter/i.test(code) && /inappropriate content/i.test(text)) {
    return "The AI provider blocked that request as inappropriate content. Please rephrase it or remove sensitive/problematic content and try again.";
  }
