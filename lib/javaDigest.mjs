...
export const JAVA_INTERVIEW_QA = [
  {
    ...
    answer: "Think of HashMap as an array of buckets, not as one large linked list. On put(key, value), Java calls key.hashCode(), spreads it with h ^ (h >>> 16) so poorly distributed hashes still affect the index, and selects a bucket with (capacity - 1) & hash because the table capacity is normally a power of two. If the bucket is empty, a node is stored. If it is occupied, HashMap compares both the hash and equals() to decide whether to replace an existing value or append a collision node. In current OpenJDK implementations, a bucket converts from a linked list to a red-black tree once it reaches 8 nodes, but only if table capacity is already at least 64; below that capacity the table resizes instead of treeifying, and a treeified bucket reverts to a list once it drops to 6 nodes. Because a red-black tree needs a total order, treeified keys are compared with Comparable when available, with a tie-break based on class name and identity hash when they are not. When size exceeds capacity × load factor (0.75 by default), the table grows, usually doubling, and entries are redistributed. During a resize, each node stays at its old index or moves by exactly the old capacity, because only one additional hash bit is now significant. A null key is stored in bucket zero. Average put/get/remove is O(1), but a pathological collision chain is O(n) before treeification and O(log n) once treeified. These are implementation details, so interview answers should distinguish the HashMap contract from version-specific OpenJDK optimizations. HashMap is not safe for concurrent structural updates; use external locking or a concurrent collection when state is shared.",
  ...
  },
  {
    ...
    answer: "The SQL standard defines four isolation levels, each permitting fewer anomalies as isolation gets stronger. Read Uncommitted allows dirty reads (seeing another transaction's uncommitted changes) and is rarely used in practice. Read Committed blocks dirty reads but still allows non-repeatable reads (a row you read once changes if you read it again in the same transaction) and phantoms (a repeated query returns different rows); it is the common default in PostgreSQL, SQL Server, and Oracle. Repeatable Read blocks dirty and non-repeatable reads; MySQL/InnoDB's implementation also blocks most phantoms via MVCC, though the SQL standard does not require that. Serializable blocks all three anomalies by making concurrent transactions behave as if run one at a time, typically at the cost of more locking, blocking, or serialization failures that require a retry. Isolation is implemented with locks, MVCC snapshots, or both, so two databases at the same level can behave differently in edge cases. Choose the weakest level that still preserves the business invariant, then back it with constraints and idempotency as additional protection.",
...
const QA_ACTION_STEPS = {
  "hashmap-internals-interview": ["Compute and spread the key hash with h ^ (h >>> 16), then select the bucket with the power-of-two index mask.", "Compare hash and equals for replacement or collision insertion; treeify only at 8+ nodes in a bucket and capacity ≥ 64, untreeify at 6.", "Resize at capacity × load factor and verify the equals/hashCode contract with immutable keys."],
...
const QA_DIAGRAM_STEPS = {
  "hashmap-internals-interview": ["Hash key & spread bits", "Mask to bucket index", "Compare hash + equals", "8+ nodes, capacity ≥64"],
  "hashmap-concurrenthashmap": ["One owner thread → HashMap", "Shared mutation → ConcurrentHashMap", "Bin-level CAS and locking", "Compound ops still need atomics"],
  "volatile-atomicity": ["Write to volatile field", "Happens-before edge to reader", "Read-modify-write still not atomic", "Use AtomicInteger or a lock"],
  "synchronized-lock": ["Acquire monitor or lock", "Keep critical section small", "synchronized: auto-release on exit", "ReentrantLock: timeout, tryLock, conditions"],
  "executor-service": ["Classify CPU vs I/O work", "Bound the pool & queue", "Submit, compose, propagate deadline", "Own shutdown & error handling"],
  "deadlock-race-condition": ["Name the shared invariant", "Reproduce with stress, not sleep", "Inspect thread dumps / lock graph", "Fix: ordering, confinement, atomics"],
  completablefuture: ["Kick off independent async calls", "Compose with thenCombine / allOf", "exceptionally vs handle diverge", "Explicit executor + timeout policy"],
  "jvm-memory-model": ["Thread stack (frames, locals)", "Shared heap (objects)", "Metaspace (class metadata)", "GC reclaims unreachable objects"],
  "jvm-class-loading": ["Locate bytecode by name", "Verify & prepare statics", "Resolve references, initialize", "Late failure → NoClassDefFoundError"],
  "spring-di-lifecycle": ["Context wires beans (DI)", "Post-processors apply proxies", "Init callbacks run", "Destroy callbacks on shutdown"],
  "spring-transactions": ["Call passes through AOP proxy", "Proxy opens transaction", "Method body runs", "Commit or rollback on exit"],
  "sql-indexes-transactions": ["Query filters on a column", "Index narrows the row scan", "Reads get faster", "Writes pay index-maintenance cost"],
  "sql-isolation-locks": ["Read Uncommitted: dirty reads", "Read Committed: blocks dirty reads", "Repeatable Read: blocks non-repeatable", "Serializable: blocks all three"],
  "system-design-reliability": ["Set a latency/availability target", "Map dependencies & finite resources", "Add timeouts, retries, circuit breaker", "Degrade gracefully on failure"],
  "system-design-scaling": ["Quantify RPS, payload, hot keys", "Scale stateless instances", "Cache safe reads with TTL", "Protect origin with rate limits"],
  "concurrency-interrupts": ["Interrupt signal is sent", "Blocking call throws InterruptedException", "Restore flag & clean up", "Stop accepting new work"],
  "concurrency-atomics": ["Thread-confined? keep it local", "One variable? atomic CAS", "Several fields? use a lock", "Measure contention either way"],
  "jvm-jit-optimizations": ["Interpreter runs bytecode first", "JIT profiles hot methods", "Compiles to machine code", "Benchmark must avoid dead-code elim"],
  "jvm-gc-tuning": ["Pick a goal: pause/throughput/footprint", "Measure allocation & live-set", "Change one variable at a time", "Cut allocation before tuning flags"],
  "spring-proxy-self-invocation": ["Call via bean → intercepted", "this.method() bypasses the proxy", "Annotation silently skipped", "Move call to another bean"],
  "spring-security-boundary": ["Authenticate at filter boundary", "Authorize action in service layer", "Check object-level ownership", "Test cross-tenant / expired creds"],
  "sql-explain-joins": ["Compare estimated vs actual rows", "Find the highest-cost plan node", "Check scan type & join order", "Fix index, stats, or query shape"],
  "sql-pagination-consistency": ["Offset: skip N rows", "Deep pages get slow & shift", "Keyset: cursor after last key", "Stable order + unique tiebreaker"],
  "system-design-consistency": ["State the invariant first", "Classify read: stale vs strong", "Strong: quorum or single writer", "Weak: async replication + conflicts"],
  "system-design-rate-limiting": ["Define identity, window, limit", "Token / leaky / sliding-window algorithm", "Shared atomic store with TTL", "Decide fail-open vs fail-closed"],
  "concurrency-threadlocal": ["Set context at request start", "Thread-bound value stored", "Pooled thread gets reused later", "Remove in finally to avoid leaks"],
  "concurrency-forkjoin": ["Split task recursively", "Idle workers steal work", "Join subtask results", "Blocking I/O starves the pool"],
  "spring-boot-autoconfiguration": ["Scan classpath & properties", "Evaluate conditions", "User bean can back off default", "Debug via condition report"],
  "spring-boot-observability": ["Instrument request/dependency spans", "Separate liveness vs readiness", "Propagate trace context", "Correlate metrics, logs, traces"],
  "spring-boot-resilience": ["Set deadline & bounded pool", "Classify retryable failures", "Retry only if idempotent", "Circuit breaker + fallback"],
  "spring-boot-testing": ["Unit test: pure logic", "Slice test: one boundary", "Integration test: real wiring", "Layered pyramid, not one suite"],
};
...
JAVA_INTERVIEW_QA.forEach((entry) => {
  ...
  Object.assign(entry, {
    ...
    diagramSteps: QA_DIAGRAM_STEPS[entry.id] || [],
  });
});
...
const SCENARIO_DIAGRAM_STEPS = {
  "prod-latency-spike": ["Confirm scope & timeline", "Compare RED signals", "Correlate CPU/GC/pool/downstream", "Mitigate: rollback, deadlines, shed load"],
  "prod-memory-growth": ["Confirm live-set growth", "Capture JFR & heap dump", "Trace roots: caches, statics, listeners", "Bound cache, add restart guardrail"],
  "prod-deadlock": ["Check thread states & locks", "Capture repeated thread dumps", "Map locks to code/order", "Break cycle, fix lock order"],
  "prod-db-regression": ["Compare latency & pool wait", "Capture actual execution plan", "Check predicates, indexes, N+1", "Add index / keyset pagination"],
  "prod-downstream-failure": ["Confirm dependency-specific failure", "Stop retries, enforce deadline", "Fail gracefully / queue for later", "Communicate & log incident ref"],
  "prod-bad-deploy": ["Compare canary vs control", "Check release diff & flags", "Halt rollout, roll back", "Add regression test post-fix"],
  "prod-high-cpu": ["Classify CPU: app/GC/kernel", "Profile hot threads/stacks", "Correlate with deploy & input", "Rollback, rate-limit, reduce work"],
  "prod-microservice-attribution": ["Start from trace/correlation ID", "Break duration into spans", "RED metrics per service", "Narrow mitigation, watch SLO"],
  "prod-slow-query": ["Capture SQL, binds, duration", "Run actual execution plan", "Check stats, locks, N+1", "Timeout, paginate, or kill query"],
  "prod-kafka-consumer-lag": ["Check lag by partition", "Inspect rebalance & poll violations", "Confirm skew & pool saturation", "Scale, batch less, route to DLQ"],
  "prod-kafka-duplicate-events": ["Find key, offset, effect ID", "Check commits & rebalances", "Add idempotency at side effect", "Replay from controlled offset"],
  "prod-mq-poison-message": ["Capture message ID & attempts", "Separate transient vs permanent failure", "Bounded retry, then quarantine/DLQ", "Alert on DLQ age, allow replay"],
  "prod-cache-stampede": ["Compare hit/miss & hot keys", "Coalesce requests, stale-while-revalidate", "Verify invalidation & eviction", "Restore TTLs gradually"],
  "prod-kubernetes-restarts": ["Inspect events, exit codes, probes", "Compare with last healthy revision", "Roll back if capacity at risk", "Isolate: app, dependency, or OOM"],
  "prod-security-incident": ["Activate incident path, preserve logs", "Revoke/rotate credential, restrict access", "Trace authz decisions & access", "Notify, scope, remediate, document"],
  "prod-docker-image": ["Inspect image digest & entrypoint", "Run image with prod-like env", "Check deps, perms, secrets", "Promote last known-good digest"],
  "prod-kubernetes-networking": ["Identify source, dest, failure type", "Check selectors, policies, DNS", "Test from a debug pod", "Restore last known-good policy"],
  "prod-kafka-rebalance": ["Measure rebalance frequency & timeouts", "Check slow I/O & poll interval", "Stabilize batch size & pause/resume", "Drain lag, verify ownership"],
  "prod-redis-failure": ["Separate latency, CPU, evictions", "Classify use: cache/session/lock", "Serve stale, reduce traffic", "Check key size & hot keys"],
  "prod-cicd-pipeline": ["Verify promoted artifact digest", "Compare config, secrets, schema", "Canary rollout with SLO gates", "Add failing check to pipeline"],
  "prod-distributed-transaction": ["Record command IDs in workflow", "Determine which effects happened", "Outbox + saga state machine", "Reconcile before reporting success"],
};

for (const scenario of JAVA_PRODUCTION_SCENARIOS) {
  scenario.diagramSteps = SCENARIO_DIAGRAM_STEPS[scenario.id] || [];
}
...
export const JAVA_EDITORIAL_CHAPTERS = {
  "Primitive types": {
    walkthrough: "Java has eight primitive types. Integral types hold whole numbers, floating-point types approximate decimals, char holds a UTF-16 code unit, and boolean holds true or false. Choose a type from the value range and required precision; an int expression can overflow rather than promote itself to long.",
    example: "int count = 2_000_000_000;\nlong safeProduct = (long) count * 3;\nSystem.out.println(safeProduct); // 6000000000",
    output: "6000000000",
    diagram: "literal → compile-time type → numeric promotion → result type",
    benchmark: "Test values at the type boundaries and measure allocation only when boxing is introduced; primitive arithmetic itself does not allocate objects.",
    mistakes: "Integer overflow, narrowing without a checked range, using float for money, and assuming char represents a complete Unicode code point.",
    interviewAnswer: "Primitive types are Java’s value types: byte, short, int, long, float, double, char, and boolean. They are not objects and have fixed language-defined ranges, so arithmetic follows binary numeric promotion and can overflow. Use long before multiplication when the result may exceed int, and use BigDecimal for decimal money calculations.",
  },
  "Wrapper classes": {
    walkthrough: "Wrapper classes represent primitive values as objects: Integer for int, Long for long, and so on. Autoboxing and unboxing make conversions concise, but unboxing null throws NullPointerException and boxing can add allocation or identity surprises.",
    example: "Integer value = 42;\nint answer = value; // unboxing\nSystem.out.println(answer);\nInteger missing = null;\n// int crash = missing; // NullPointerException",
    output: "42",
    diagram: "primitive value ↔ boxing/unboxing conversion ↔ wrapper object",
    benchmark: "Compare primitive collections or arrays with boxed collections under allocation pressure; inspect heap allocation rather than relying on source-level syntax.",
    mistakes: "Comparing wrappers with ==, unboxing null, and assuming Integer identity is a value comparison.",
    interviewAnswer: "Wrappers let primitive values participate in APIs that require objects, such as List<Integer>. Java inserts boxing and unboxing conversions where allowed. Compare wrapper values with equals or safely unbox after a null check; == compares references when both operands are wrappers.",
  },
  "Casting and promotion": {
    walkthrough: "Widening conversions preserve every value and happen implicitly; narrowing conversions can lose information and require an explicit cast. Before arithmetic, Java promotes byte, short, and char operands to int, which is why assigning their sum back to byte needs a cast.",
    example: "byte left = 100;\nbyte right = 27;\nint sum = left + right;\nbyte narrowed = (byte) sum;\nSystem.out.println(sum); // 127",
    output: "127",
    diagram: "operand types → binary numeric promotion → expression type → optional narrowing",
    benchmark: "Test min/max values and fractional truncation; correctness at boundaries matters more than microbenchmark differences between primitive widths.",
    mistakes: "Casting after overflow, truncating decimals unintentionally, and confusing reference casts with numeric casts.",
    interviewAnswer: "Widening is an implicit conversion such as int to long. Narrowing, such as long to int or double to int, is explicit because it can lose range or precision. For arithmetic, byte and short are promoted to int first, so cast only after proving the result fits.",
  },
  "Operators": {
    walkthrough: "Operators combine values and produce a result. Precedence determines grouping, short-circuit && and || may skip the right operand, and == compares primitive values but object references. Parenthesize mixed expressions when the intended evaluation order matters.",
    example: "boolean allowed = user != null && user.isActive();\nint total = (basePrice * quantity) - discount;\nSystem.out.println(allowed && total > 0);",
    output: "true when an active user has a positive total",
    diagram: "operands → precedence/short-circuit rule → evaluated expression → value",
    benchmark: "Benchmark only meaningful alternative algorithms; operator-level microbenchmarks rarely represent application performance.",
    mistakes: "Using & instead of && for guards, comparing Strings with ==, and relying on obscure precedence.",
    interviewAnswer: "Java operators include arithmetic, comparison, logical, bitwise, assignment, and conditional operators. && and || short-circuit, which is useful for null-safe guards. Use equals for object value comparison and parentheses to make mixed precedence explicit.",
  },
  "Control flow": {
    walkthrough: "Control flow chooses or repeats execution. A loop should have a clear progress measure and termination condition; switch expressions can return a value, while break and continue alter the nearest loop or switch.",
    example: "int sum = 0;\nfor (int value : new int[] {2, 4, 6}) {\n  sum += value;\n}\nSystem.out.println(sum); // 12",
    output: "12",
    diagram: "condition/input → branch or loop body → state update → next condition",
    benchmark: "Measure complete workloads, including input size and allocation; choose the clearest construct with the required asymptotic behavior.",
    mistakes: "Off-by-one bounds, accidental infinite loops, fall-through in classic switch, and mutating a collection during an enhanced for loop.",
    interviewAnswer: "Use if for conditions, switch for a finite set of alternatives, and for/while loops for repetition. Every loop needs a termination argument. Enhanced for is concise for traversal, but an explicit iterator or index is needed when the traversal mutates state.",
  },
  "Methods and parameters": {
    walkthrough: "A method declares a name, parameters, return type, and body. Java passes arguments by value—including object references—so a method can mutate a referenced object but cannot replace the caller’s reference. Keep methods focused and make invalid inputs explicit.",
    example: "static void addTag(List<String> tags) {\n  tags.add(\"java\");\n}\nArrayList<String> tags = new ArrayList<>();\naddTag(tags);\nSystem.out.println(tags); // [java]",
    output: "[java]",
    diagram: "caller argument value → parameter → method body → return or observable mutation",
    benchmark: "Measure public behavior at realistic call volume; avoid optimizing a method before profiling its caller and allocations.",
    mistakes: "Calling reference passing ‘pass by reference’, mutable global state, unclear null contracts, and methods that mix unrelated responsibilities.",
    interviewAnswer: "Java is always pass-by-value. For an object argument, the copied value is a reference, so the method can change the object but reassigning its parameter does not change the caller’s variable. A method’s parameter and return types should express its contract directly.",
  },
  "Access modifiers": {
    walkthrough: "public, protected, package-private, and private control which code can access a type or member. Prefer the narrowest visibility that supports the contract; encapsulation lets an implementation change without breaking callers.",
    example: "public final class Counter {\n  private int value;\n  public int increment() { return ++value; }\n}\nSystem.out.println(new Counter().increment()); // 1",
    output: "1",
    diagram: "caller/package/subclass → visibility rule → accessible API surface",
    benchmark: "Visibility is a design boundary, not a performance knob; measure behavior only after choosing a stable API.",
    mistakes: "Public mutable fields, using protected as a convenience channel, and assuming package-private members cross packages.",
    interviewAnswer: "private is class-only, package-private is the default and stays within the package, protected permits the package plus subclass access, and public exposes the API. Use private fields with intentional methods to preserve invariants and reduce coupling.",
  },
  "Constructors": {
    walkthrough: "A constructor establishes an object’s invariants before the object escapes. Constructor chaining with this or super reuses initialization, and a class with no declared constructor receives a default no-argument constructor only when the compiler can provide one.",
    example: "final class Port {\n  private final int value;\n  Port(int value) {\n    if (value <= 0) throw new IllegalArgumentException(\"port\");\n    this.value = value;\n  }\n}\nnew Port(8080);",
    output: "A Port can never be created with a non-positive value",
    diagram: "arguments → validation → field assignment → fully initialized object",
    benchmark: "Validate construction cost only when creating very large object graphs; correctness and invariant enforcement come first.",
    mistakes: "Leaking this during construction, duplicating validation across overloads, and assuming constructors are inherited.",
    interviewAnswer: "Constructors initialize a new instance and cannot declare a return type. Use one canonical constructor for required state, delegate overloads with this(...), and validate before assigning fields. Constructors are not inherited, although subclasses must invoke a superclass constructor.",
  },
  "Inheritance": {
    walkthrough: "Inheritance models an is-a relationship and enables substitutability through overriding. A subclass inherits accessible behavior but should preserve the superclass contract; composition is often safer when the relationship is implementation reuse rather than a true subtype.",
    example: "class SavingsAccount extends Account {\n  @Override\n  Money fee() { return Money.zero(); }\n}\nAccount account = new SavingsAccount();\nSystem.out.println(account.fee()); // 0",
    output: "0",
    diagram: "supertype contract → subtype override → dynamic dispatch → behavior",
    benchmark: "Compare designs by domain correctness and change cost; inheritance rarely provides a meaningful runtime advantage over composition.",
    mistakes: "Overriding without @Override, breaking preconditions, deep hierarchies, and calling overridable methods from constructors.",
    interviewAnswer: "Inheritance lets a subtype reuse and specialize a superclass while remaining usable where the superclass is expected. Dynamic dispatch selects an overridden instance method at runtime. Apply Liskov substitutability: the subtype must honor the parent’s observable contract.",
  },
  "Composition": {
    walkthrough: "Composition builds behavior by holding collaborators and delegating to them. It keeps dependencies replaceable and avoids the fragile coupling of deep inheritance trees; the owner controls which collaborators participate in its invariant.",
    example: "final class Checkout {\n  private final TaxPolicy tax;\n  Checkout(TaxPolicy tax) { this.tax = tax; }\n  Money total(Money subtotal) { return subtotal.plus(tax.forAmount(subtotal)); }\n}",
    output: "Checkout delegates tax calculation without inheriting TaxPolicy",
    diagram: "owner → collaborator interface → delegated behavior → composed result",
    benchmark: "Compare replacement seams and object graph size; composition pays a small indirection cost for lower change coupling.",
    mistakes: "God objects, exposing collaborators, hidden service locators, and composing concrete classes that cannot be replaced in tests.",
    interviewAnswer: "Composition means an object contains collaborators and delegates work to them. It favors has-a relationships, keeps responsibilities small, and allows different policies to be injected. Prefer it when subclassing would expose implementation details or violate substitutability.",
  },
  "Abstract classes": {
    walkthrough: "An abstract class shares state or implementation while leaving one or more operations for subclasses. It cannot be instantiated directly. Use it when the base class owns a genuine invariant and a common lifecycle; use an interface when you need an independently composable capability.",
    example: "abstract class Parser {\n  final String source;\n  Parser(String source) { this.source = source; }\n  abstract int parse();\n}\nfinal class CountParser extends Parser {\n  CountParser(String source) { super(source); }\n  int parse() { return source.length(); }\n}",
    output: "A CountParser reuses base state and supplies parse()",
    diagram: "abstract invariant → subclass implementation → polymorphic operation",
    benchmark: "Choose an abstraction based on change coupling and test seams; inheritance dispatch is rarely the limiting cost.",
    mistakes: "Putting unrelated behavior in the base class, calling overridable methods from constructors, and using abstract classes only to share constants.",
    interviewAnswer: "An abstract class can hold fields, constructors, concrete methods, and abstract methods, but cannot be instantiated. It is appropriate when related subclasses share state and a lifecycle. Interfaces are usually better for capabilities that may be combined across unrelated classes.",
  },
  "Interfaces": {
    walkthrough: "An interface defines a contract that unrelated classes can implement. It can contain abstract methods plus default and static methods, but it does not own per-instance state. Depend on interfaces at boundaries so implementations can be replaced.",
    example: "interface Formatter {\n  String format(String value);\n}\nfinal class UpperCaseFormatter implements Formatter {\n  public String format(String value) { return value.toUpperCase(); }\n}\nSystem.out.println(new UpperCaseFormatter().format(\"java\")); // JAVA",
    output: "JAVA",
    diagram: "consumer → interface contract ← implementation",
    benchmark: "Measure the implementation’s real work; interface dispatch is not a reason to duplicate a stable boundary.",
    mistakes: "Leaking implementation details, huge interfaces, default methods that hide breaking behavior, and using marker interfaces as a substitute for documentation.",
    interviewAnswer: "An interface is a polymorphic contract. A class can implement multiple interfaces, which supports composition of capabilities without multiple class inheritance. Keep interfaces focused and inject them where callers need behavior rather than a concrete implementation.",
  },
  "Polymorphism": {
    walkthrough: "Polymorphism lets code use a supertype while the runtime selects an overriding implementation. The declared type controls what members are callable; the object’s runtime type controls overridden instance behavior. This separates policy from concrete variants.",
    example: "interface Payment { String authorize(); }\nPayment payment = new CardPayment();\nSystem.out.println(payment.authorize()); // card-approved",
    output: "card-approved",
    diagram: "declared contract → runtime receiver → dynamic dispatch → variant behavior",
    benchmark: "Compare complete strategies under realistic load; polymorphic calls are usually cheaper than the maintenance cost of condition-heavy branching.",
    mistakes: "Downcasting to recover lost behavior, checking class names, violating the shared contract, and replacing polymorphism with a growing switch.",
    interviewAnswer: "Polymorphism means one contract can represent multiple implementations. With overriding, Java uses dynamic dispatch for instance methods, so a Payment reference can invoke CardPayment behavior. The caller depends on the stable contract, not the concrete type.",
  },
  "Enums": {
    walkthrough: "An enum defines a closed set of named instances. Enum constants can carry fields and behavior, making them safer than string flags. Persist a stable external code when values cross a database or API boundary; do not persist ordinal positions.",
    example: "enum Status { NEW, PAID, CANCELLED }\nStatus status = Status.PAID;\nSystem.out.println(status.name()); // PAID",
    output: "PAID",
    diagram: "closed domain values → enum instance → exhaustive branch or behavior",
    benchmark: "Enum lookup is already efficient; focus on stable serialization and domain clarity rather than micro-optimizing comparisons.",
    mistakes: "Persisting ordinal(), comparing enum names from untrusted input without validation, and using an enum for an open-ended database vocabulary.",
    interviewAnswer: "Enums are classes with a fixed set of instances. They provide type safety, can have fields and methods, and work naturally with switch. Never use ordinal as a durable identifier; map an explicit code when serialization must remain compatible.",
  },
  "Records": {
    javaVersions: "16+",
    walkthrough: "A record is a concise, final data carrier. The compiler supplies a canonical constructor, accessors, equals, hashCode, and toString for its components. Records are shallowly immutable: a referenced list inside a record can still be mutable.",
    example: "record Point(int x, int y) {}\nPoint point = new Point(3, 4);\nSystem.out.println(point.x() + point.y()); // 7",
    output: "7",
    diagram: "components → generated constructor/accessors/equality → value carrier",
    benchmark: "Use records for modeling and API clarity; benchmark serialization or allocation only when profiling identifies it as material.",
    mistakes: "Calling records deeply immutable, trying to extend them, using them for identity-heavy entities, and assuming accessor names follow getX().",
    interviewAnswer: "Records, permanent since Java 16, reduce boilerplate for transparent data carriers. They are implicitly final and provide component accessors plus value-oriented equality. Their immutability is shallow, so mutable component objects still need defensive handling.",
  },
  "Sealed classes": {
    javaVersions: "17+",
    walkthrough: "A sealed class or interface restricts which types may directly extend or implement it. The compiler can then check exhaustiveness in supported pattern switches. Use sealing when the domain is intentionally closed and owned by the same module or package boundary.",
    example: "sealed interface Result permits Success, Failure {}\nrecord Success(String value) implements Result {}\nrecord Failure(String reason) implements Result {}",
    output: "Only Success and Failure can implement Result",
    diagram: "sealed contract → permitted subtypes → exhaustive domain handling",
    benchmark: "Sealing is a modeling and compile-time safety feature; measure the application behavior, not the declaration itself.",
    mistakes: "Sealing an extension point that consumers must implement, forgetting permitted subtype visibility rules, and treating compile-time exhaustiveness as runtime validation.",
    interviewAnswer: "Sealed types make an inheritance hierarchy explicit by listing permitted direct subtypes. This supports closed-domain modeling and compiler checks, especially with records and pattern matching. They are a poor fit for a plugin API designed for third-party extension.",
  },
  "String immutability": {
    walkthrough: "String objects cannot change after construction. Operations such as concat return a new value, which makes strings safe to share and allows pooling, but repeated concatenation in a loop can create unnecessary intermediates.",
    example: "String prefix = \"java\";\nString result = prefix.concat(\"-21\");\nSystem.out.println(prefix); // java\nSystem.out.println(result); // java-21",
    output: "java\njava-21",
    diagram: "original String → non-mutating operation → new String value",
    benchmark: "For repeated assembly, compare StringBuilder with immutable concatenation at realistic sizes and inspect allocation rate.",
    mistakes: "Expecting concat to mutate, using == for content, and assuming intern() is a general-purpose memory optimization.",
    workedExample: "Start with prefix = \"java\"; concat creates a second object, so prefix remains \"java\" while result becomes \"java-21\". In a loop, each concat can create another intermediate value.",
    why: "A String's value and hash code stay stable after construction, making sharing, pooling, and use as a map key safe.",
    failure: "Immutability does not prevent memory pressure: repeated concatenation, unbounded input, or careless interning can still allocate or retain too much.",
    complexity: "A single concat copies characters into a new String; repeated loop concatenation can become O(n²). StringBuilder reduces assembly to amortized O(n).",
    edgeCases: "Empty strings, null versus \"null\", Unicode code points, content comparison with equals, and very large concatenations.",
    realWorldAnalogy: "Intuition only: a printed label cannot be edited in place; to change the wording you print a new label. That makes sharing safe, but printing thousands of revised labels wastes material.",
    interviewAnswer: "String is immutable, so every apparent modification produces another String. Immutability makes sharing, caching, and hash codes safe, but repeated concatenation can allocate many objects. Use StringBuilder for loop-based assembly and equals for content comparison.",
  },
  "StringBuilder": {
    walkthrough: "StringBuilder is a mutable character buffer for building one result through many appends. It avoids creating an intermediate String per append and is not synchronized; use StringBuffer only when its legacy synchronization contract is specifically required.",
    example: "StringBuilder builder = new StringBuilder();\nfor (int i = 1; i <= 3; i++) builder.append(i);\nSystem.out.println(builder.toString()); // 123",
    output: "123",
    diagram: "mutable buffer → append operations → final toString snapshot",
    benchmark: "Compare allocation and elapsed time for repeated concatenation versus StringBuilder over 10, 1k, and 1M appends.",
    mistakes: "Sharing one builder across threads, calling toString too early, and using it when one compile-time concatenation is clearer.",
    workedExample: "After append(1), the buffer is \"1\"; after append(2), it is \"12\"; after append(3), it is \"123\". toString() takes an immutable snapshot of that final state.",
    why: "One owned mutable buffer absorbs appends, avoiding an intermediate String for every step while preserving a clear final conversion boundary.",
    failure: "It fails when a builder is shared across threads, retained without bounds, reused after its contents should be isolated, or converted toString before all appends finish.",
    complexity: "Appending is amortized O(1) per character and O(n) overall for n output characters; occasional capacity growth copies the buffer. toString is O(n).",
    edgeCases: "Empty builder, null append values, capacity growth, Unicode surrogate pairs, very large output, and accidental cross-thread sharing.",
    realWorldAnalogy: "Intuition only: use one notepad while drafting a message, then photocopy the finished page when it is ready to share. The notepad is efficient, but two writers need separate ownership.",
    interviewAnswer: "StringBuilder provides a mutable buffer for efficient sequential string construction. Appends mutate the builder and toString creates the final immutable String. It is unsynchronized, so do not share it across threads without external coordination.",
  },
  "Arrays": {
    walkthrough: "An array has a fixed length and stores values of one component type. Indexes run from zero through length minus one. Arrays are useful for compact, predictable storage; use a collection when size changes or richer operations are required.",
    example: "int[] scores = {7, 8, 9};\nint total = 0;\nfor (int score : scores) total += score;\nSystem.out.println(total); // 24",
    output: "24",
    diagram: "fixed length → zero-based index → element read/write",
    benchmark: "Compare arrays and collections for the workload’s access pattern, memory density, resizing needs, and boxing behavior.",
    mistakes: "Using an invalid index, confusing length with a method, aliasing the same array, and assuming Arrays.asList creates a resizable list.",
    interviewAnswer: "Arrays are fixed-size, zero-indexed containers whose component type is enforced at runtime. They provide constant-time indexed access and low overhead, but cannot grow. Choose a collection when you need resizing, searching helpers, or a collection API.",
  },
  "ArrayList": {
    walkthrough: "ArrayList is a resizable array-backed List. Indexed reads are constant time; appending is amortized constant time, while inserting or removing in the middle shifts later elements. Size growth may copy the backing array.",
    example: "List<String> names = new ArrayList<>();\nnames.add(\"Ada\");\nnames.add(\"Linus\");\nSystem.out.println(names.get(1)); // Linus",
    output: "Linus",
    diagram: "logical list → contiguous backing array → index lookup or shifted insertion",
    benchmark: "Compare indexed access, middle insertion, and iteration separately; include resizing and allocation in the measurement.",
    mistakes: "Removing while iterating incorrectly, assuming add(0, value) is cheap, and exposing a mutable list when callers should receive an unmodifiable view.",
    interviewAnswer: "ArrayList stores elements in a growable array. get and set are O(1), append is amortized O(1), and middle insertion/removal is O(n) because elements shift. It is a strong default for read-heavy, indexable lists.",
  },
  "LinkedList": {
    walkthrough: "LinkedList stores each element in a node linked to its neighbors. Traversal to an index is linear, and insertion is cheap only after the correct node has already been located. In most application code ArrayList or ArrayDeque has better locality.",
    example: "Deque<String> queue = new LinkedList<>();\nqueue.addLast(\"first\");\nqueue.addLast(\"second\");\nSystem.out.println(queue.removeFirst()); // first",
    output: "first",
    diagram: "node links → traversal to position → link update → list state",
    benchmark: "Measure the actual access pattern and cache behavior; do not infer performance from O(1) node insertion alone.",
    mistakes: "Using get(i) in a loop, choosing it automatically for queues, and retaining removed nodes through accidental references.",
    interviewAnswer: "LinkedList offers node-based insertion and removal but poor indexed access and locality. Its Deque operations are valid, yet ArrayDeque is usually faster for a queue or stack. Choose it only when its node semantics fit a measured workload.",
  },
  "HashSet": {
    walkthrough: "HashSet stores unique elements using hashing. add and contains are expected O(1), but correctness depends on stable equals and hashCode values while an element is in the set. Iteration order is not a contract.",
    example: "Set<String> seen = new HashSet<>();\nseen.add(\"java\");\nseen.add(\"java\");\nSystem.out.println(seen.size()); // 1",
    output: "1",
    diagram: "element → hashCode → bucket → equals collision check → uniqueness",
    benchmark: "Measure contains under realistic load factors and collision behavior; compare with a sorted set only when ordering is required.",
    mistakes: "Mutating a key after insertion, expecting stable iteration order, and using a set when duplicate occurrences carry meaning.",
    interviewAnswer: "HashSet uses a hash table to enforce uniqueness. Equality and hashing determine whether an element is already present. Average add and contains are constant time, but order is unspecified and mutable elements can become unreachable after their hash changes.",
  },
  "TreeSet": {
    walkthrough: "TreeSet maintains unique elements in sorted order using a balanced tree. Operations are O(log n), and the ordering must be consistent with equality for predictable set behavior. It is useful when range and ordered traversal matter.",
    example: "NavigableSet<Integer> scores = new TreeSet<>(Arrays.asList(30, 10, 20));\nSystem.out.println(scores.first() + \"-\" + scores.last()); // 10-30",
    output: "10-30",
    diagram: "element → comparator ordering → balanced tree → sorted/range view",
    benchmark: "Compare log-time ordered operations with hash lookup; include comparator cost and range-query frequency.",
    mistakes: "Comparators that return zero for distinct values, mutating ordered fields, and expecting hash-table constant-time lookup.",
    interviewAnswer: "TreeSet is a sorted Set backed by a navigable balanced tree. add, remove, and contains are O(log n), and methods such as ceiling and subSet provide ordered queries. Its comparator defines set identity, so inconsistent comparison can discard values.",
  },
  "TreeMap": {
    walkthrough: "TreeMap stores key-value pairs ordered by natural ordering or a Comparator. It provides O(log n) lookup and navigable range operations. The comparator must be consistent with the map’s intended key identity.",
    example: "NavigableMap<Integer, String> levels = new TreeMap<>();\nlevels.put(10, \"low\");\nlevels.put(20, \"high\");\nSystem.out.println(levels.floorEntry(15).getValue()); // low",
    output: "low",
    diagram: "key → comparator → balanced tree node → ordered lookup/range result",
    benchmark: "Compare TreeMap with HashMap plus a separate sort when range queries and ordered iteration are or are not required.",
    mistakes: "Using mutable keys, assuming insertion order, and writing a comparator that violates transitivity.",
    interviewAnswer: "TreeMap is a sorted NavigableMap with O(log n) basic operations. Unlike HashMap, it supports floor, ceiling, first/last, and range views. Ordering comes from the comparator or Comparable keys and must remain consistent.",
  },
  "PriorityQueue": {
    walkthrough: "PriorityQueue is a heap: peek returns the least element according to its ordering, and poll removes that element. Iterating it does not produce sorted order; repeated poll is the way to consume in priority order.",
    example: "PriorityQueue<Integer> work = new PriorityQueue<>();\nwork.add(30); work.add(10); work.add(20);\nSystem.out.println(work.poll()); // 10",
    output: "10",
    diagram: "offer → heap invariant → peek/poll root → reheapify",
    benchmark: "Measure offer and poll at realistic queue sizes; compare a bounded heap with full sorting when only the top k items are needed.",
    mistakes: "Expecting iteration to be sorted, mutating an element’s priority in place, and reversing priority accidentally with a comparator.",
    interviewAnswer: "PriorityQueue is a binary heap. offer and poll are O(log n), peek is O(1), and iteration has no sorted-order guarantee. It is ideal for repeatedly selecting the next minimum or maximum item, not for arbitrary sorted traversal.",
  },
  "ArrayDeque": {
    walkthrough: "ArrayDeque is a resizable circular array supporting efficient insertion and removal at both ends. It is the usual choice for a single-threaded stack or queue; it rejects null so null can safely represent an absent result.",
    example: "Deque<String> tasks = new ArrayDeque<>();\ntasks.addLast(\"compile\");\ntasks.addFirst(\"checkout\");\nSystem.out.println(tasks.removeFirst()); // checkout",
    output: "checkout",
    diagram: "head/tail indexes → circular buffer → add/remove at either end",
    benchmark: "Compare ArrayDeque with LinkedList for queue workloads; include allocation, locality, and peak size.",
    mistakes: "Adding null, using it concurrently without synchronization, and calling remove on an empty deque instead of a poll method when absence is valid.",
    interviewAnswer: "ArrayDeque is a non-thread-safe Deque backed by a resizable circular array. End operations are amortized O(1), and it usually outperforms LinkedList for stacks and queues because of memory locality. It does not permit null elements.",
  },
  "equals and hashCode": {
    walkthrough: "equals defines logical equality; hashCode must return the same value for equal objects. Hash-based collections rely on both methods, so fields used by the contract must not change while the object is a key or set element.",
    example: "Set<String> ids = new HashSet<>();\nids.add(\"u-7\");\nSystem.out.println(ids.contains(new String(\"u-7\"))); // true",
    output: "true",
    diagram: "candidate → hashCode bucket → equals confirmation → logical identity",
    benchmark: "Measure collision rates only after the equality contract is correct; correctness failures are more damaging than hash distribution differences.",
    mistakes: "Overriding one method without the other, using mutable fields in a key, and comparing floating-point fields naively.",
    interviewAnswer: "The equals contract is reflexive, symmetric, transitive, consistent, and false for null. If a.equals(b) is true, a.hashCode() must equal b.hashCode(). HashMap and HashSet use the hash first and equals to resolve collisions, so mutable identity fields are unsafe keys.",
  },
  "Comparable and Comparator": {
    walkthrough: "Comparable gives a type one natural ordering through compareTo. Comparator supplies an ordering externally, allowing multiple views. A comparator should be transitive and, when used by sorted collections, consistent with the equality semantics you want.",
    example: "List<String> names = new ArrayList<>(Arrays.asList(\"Bob\", \"Ada\"));\nnames.sort(Comparator.comparingInt(String::length));\nSystem.out.println(names); // [Bob, Ada]",
    output: "[Bob, Ada]",
    diagram: "two values → comparison result (-/0/+) → ordering decision",
    benchmark: "Include comparator computation and sort size; avoid repeated expensive key extraction by comparing precomputed keys where appropriate.",
    mistakes: "Subtracting ints and overflowing, returning zero for unequal values in a TreeSet, and mixing ascending and descending rules.",
    interviewAnswer: "Comparable defines a class’s natural order; Comparator defines an independent order and can be passed to sorting or ordered collections. Compare with Integer.compare rather than subtraction, and document whether comparison is consistent with equals.",
  },
  "Wildcards and PECS": {
    walkthrough: "Use ? extends T when a parameter produces T values and ? super T when it consumes T values. PECS—Producer Extends, Consumer Super—describes what the caller may safely read or write through the wildcard boundary.",
    example: "static void copy(List<? extends Number> source, List<? super Number> target) {\n  target.addAll(source);\n}",
    output: "A producer list can be read and a consumer list can accept Number values",
    diagram: "generic boundary → permitted reads/writes → compile-time type safety",
    benchmark: "Wildcards affect compile-time flexibility, not collection runtime performance; benchmark the concrete collection operations.",
    mistakes: "Trying to add to an extends list, reading a super list as a specific subtype, and using wildcards when a type parameter expresses a relationship better.",
    interviewAnswer: "? extends T is safe for reading values as T but not adding arbitrary T values. ? super T is safe for adding T values but reads only as Object. Use a named type parameter when two positions must refer to the same exact type.",
  },
  "Type erasure": {
    javaVersions: "8+",
    walkthrough: "Java generics are primarily a compile-time feature. The compiler checks type arguments and erases most generic type information from ordinary bytecode, inserting casts where needed. That is why new T(), List<String>.class, and instanceof List<String> are not allowed.",
    example: "List<String> names = new ArrayList<>();\nnames.add(\"Ada\");\nString name = names.get(0); // compiler inserts a checked cast\nSystem.out.println(name); // Ada",
    output: "Ada",
    diagram: "generic source → compile-time checks/casts → erased class representation → runtime object",
    benchmark: "Erasure is not a performance strategy; compare boxing, allocation, and concrete collection choices in the generated workload.",
    mistakes: "Expecting runtime generic type checks, heap-polluting raw types, and assuming overloads can differ only by generic arguments.",
    interviewAnswer: "Type erasure preserves source compatibility with pre-generics Java by removing most type arguments after compilation and inserting casts at use sites. Reifiable types such as String[] retain runtime component information, while List<String> does not. Use a Class token or explicit type metadata when runtime type information is required.",
  },
  "Checked exceptions": {
    javaVersions: "11+",
    walkthrough: "A checked exception is part of a method’s compile-time contract: callers must catch it or declare it. Use checked exceptions for recoverable conditions that callers can reasonably handle, and preserve the cause when translating layers.",
    example: "static String read(Path path) throws IOException {\n  return Files.readString(path);\n}\ntry {\n  System.out.println(read(Path.of(\"config.txt\")));\n} catch (IOException error) {\n  System.err.println(\"Configuration unavailable\");\n}",
    output: "The caller handles an unavailable file explicitly",
    diagram: "operation → checked failure contract → caller recovery or propagation",
    benchmark: "Measure failure frequency and recovery latency; exception construction should not be used as ordinary loop control.",
    mistakes: "Catching Exception broadly, swallowing the cause, logging and rethrowing at every layer, and forcing callers to handle unrecoverable programming errors.",
    interviewAnswer: "Checked exceptions must be caught or declared, so they document recoverable failure at the API boundary. Catch the narrowest type, add useful context while preserving the cause, and choose unchecked exceptions for violations callers cannot meaningfully recover from.",
  },
  "Unchecked exceptions": {
    walkthrough: "Unchecked exceptions extend RuntimeException or Error and do not need a throws declaration. Illegal arguments, invalid state, and null misuse commonly belong here. They still need a clear boundary and safe handling at the application edge.",
    example: "static int percentage(int value) {\n  if (value < 0 || value > 100) throw new IllegalArgumentException(\"0..100\");\n  return value;\n}\nSystem.out.println(percentage(75)); // 75",
    output: "75",
    diagram: "invalid state/input → unchecked exception → boundary maps failure safely",
    benchmark: "Measure valid-path behavior; validate input before expensive work and do not use exceptions for expected branching.",
    mistakes: "Returning null instead of a clear contract, catching RuntimeException to continue corrupted work, and exposing stack traces to users.",
    interviewAnswer: "Unchecked exceptions signal programming errors or invalid state without forcing every caller to catch them. They should be handled at an appropriate boundary, translated to safe responses, and not used for routine expected control flow.",
  },
  "try-with-resources": {
    javaVersions: "11+",
    walkthrough: "Try-with-resources closes every AutoCloseable resource in reverse declaration order, even when the body throws. If close also fails, Java preserves that failure as a suppressed exception on the primary exception.",
    example: "try (BufferedReader reader = Files.newBufferedReader(Path.of(\"input.txt\"))) {\n  System.out.println(reader.readLine());\n} // reader closes here",
    output: "The reader is closed on success and failure",
    diagram: "acquire → use → primary failure? → reverse-order close → suppressed failures",
    benchmark: "Measure resource lifetime and I/O throughput, not the negligible syntax overhead; verify descriptors and connections are released under failure.",
    mistakes: "Manually closing in a finally block, declaring resources in the wrong order, and losing suppressed exceptions while replacing the primary cause.",
    interviewAnswer: "Try-with-resources manages AutoCloseable resources deterministically. Resources close in reverse order, and close failures are suppressed if the body already failed. It is safer and easier to review than hand-written finally cleanup.",
  },
  "Custom exceptions": {
    walkthrough: "A custom exception names a domain failure that callers can handle differently. Keep it small, include stable machine-readable context when needed, and choose checked versus unchecked based on recoverability rather than naming preference.",
    example: "final class InsufficientFundsException extends RuntimeException {\n  InsufficientFundsException(String message) { super(message); }\n}\nthrow new InsufficientFundsException(\"balance too low\");",
    output: "A domain-specific failure reaches the service boundary",
    diagram: "domain rule → named exception → boundary translation → safe client outcome",
    benchmark: "Measure normal business flow; test exception paths for latency and logging volume without leaking sensitive data.",
    mistakes: "Creating dozens of indistinguishable types, exposing internal IDs in messages, and discarding the original cause when wrapping.",
    interviewAnswer: "Use a custom exception when a domain failure has distinct recovery, mapping, or observability needs. Give it constructors that preserve causes and structured context, then translate it at the boundary instead of coupling clients to internals.",
  },
  "Lambda expressions": {
    walkthrough: "A lambda is an expression that implements a functional interface. Capture only effectively final local variables, keep side effects explicit, and prefer a named method when the behavior deserves a reusable name or stack-trace clarity.",
    example: "List<String> names = Arrays.asList(\"Ada\", \"Linus\");\nnames.sort((left, right) -> left.compareToIgnoreCase(right));\nSystem.out.println(names); // [Ada, Linus]",
    output: "[Ada, Linus]",
    diagram: "target functional interface → lambda body → invocation",
    benchmark: "Benchmark the surrounding allocation and traversal; lambda syntax itself is not a reason to choose a slower pipeline.",
    mistakes: "Mutating shared state inside lambdas, hiding checked exceptions, capturing changing loop variables, and using nested lambdas that cannot be read.",
    interviewAnswer: "A lambda supplies the single abstract method of a functional interface, enabling behavior to be passed as data. It can capture effectively final variables. Keep lambdas short and side-effect free where possible, and use method references when they improve clarity.",
  },
  "Functional interfaces": {
    walkthrough: "A functional interface has exactly one abstract method, although it may have default and static methods. @FunctionalInterface asks the compiler to enforce that contract. Java’s Function, Predicate, Consumer, and Supplier cover common shapes.",
    example: "Predicate<String> nonBlank = value -> value != null && !value.trim().isEmpty();\nSystem.out.println(nonBlank.test(\"java\")); // true",
    output: "true",
    diagram: "one abstract method → lambda/method reference → reusable behavior",
    benchmark: "Choose the interface that makes the contract clear; measure the operation being passed, not the abstraction label.",
    mistakes: "Adding a second abstract method, using Consumer when a result is needed, and making callbacks responsible for unrelated state changes.",
    interviewAnswer: "A functional interface is a target type for a lambda or method reference and has one abstract method. Standard interfaces such as Predicate<T> and Function<T,R> reduce boilerplate; @FunctionalInterface prevents accidental contract drift.",
  },
  "Collectors": {
    walkthrough: "Collectors reduce a stream into a result such as a List, Map, grouping, partition, or summary. Choose the collector from the desired result shape and account for duplicate keys, ordering, and null policy explicitly.",
    example: "Map<String, Long> counts = names.stream()\n    .collect(Collectors.groupingBy(String::toLowerCase, Collectors.counting()));",
    output: "A map containing each normalized name and its count",
    diagram: "stream elements → collector accumulation → combination → final result",
    benchmark: "Compare collector allocation and merge cost on realistic data; parallel collection is useful only when work and combiner behavior justify it.",
    mistakes: "toMap without a duplicate-key merge function, relying on unspecified map order, and collecting huge streams into unbounded memory.",
    interviewAnswer: "Collectors describe a mutable reduction from stream elements to a result. groupingBy, partitioningBy, mapping, reducing, and toMap cover common shapes. Specify merge, map type, ordering, and memory behavior rather than assuming defaults.",
  },
  "Optional": {
    walkthrough: "Optional models a possibly absent return value and makes absence visible to callers. It is most useful at return boundaries; avoid using it for fields, method parameters, or serialization unless the surrounding framework explicitly supports that contract.",
    example: "Optional<String> nickname = Optional.ofNullable(profile.nickname());\nString label = nickname.orElse(\"anonymous\");\nSystem.out.println(label);",
    output: "The nickname, or anonymous when it is absent",
    diagram: "possibly absent value → Optional container → map/filter/orElse decision",
    benchmark: "Do not microbenchmark Optional syntax; measure the actual lookup and avoid creating nested optionals in hot paths without evidence.",
    mistakes: "Calling get without checking, using orElse when the fallback is expensive, and returning null from an Optional-returning method.",
    interviewAnswer: "Optional communicates that a return may be absent and encourages an explicit policy such as map, filter, orElse, or orElseThrow. It is not a universal null replacement; keep it at API boundaries and return Optional.empty rather than null.",
  },
  "Date and time API": {
    walkthrough: "The java.time API separates machine time from human time: Instant is a timeline point, LocalDate is a date without a zone, and ZonedDateTime includes zone rules. Types are immutable and thread-safe; choose the type from the business meaning.",
    example: "LocalDate due = LocalDate.of(2026, 9, 4);\nLocalDate next = due.plusDays(7);\nSystem.out.println(next); // 2026-09-11",
    output: "2026-09-11",
    diagram: "business meaning → java.time type → explicit zone/format → stable value",
    benchmark: "Measure parsing and formatting only when they are hot; correctness around DST, offsets, and locale dominates micro-optimizations.",
    mistakes: "Using LocalDateTime for an instant, applying the system default zone implicitly, and using legacy Date/SimpleDateFormat concurrently.",
    interviewAnswer: "java.time provides immutable, thread-safe temporal types. Use Instant for an event timestamp, LocalDate for a date-only business value, and ZonedDateTime when a zone’s rules matter. Store and exchange an explicit offset or UTC policy rather than relying on the host default.",
  },
  "Threads": {
    walkthrough: "A platform thread executes a task concurrently, but shared mutable state still needs a visibility and atomicity strategy. Prefer higher-level executors, immutable data, or message passing; if sharing state, define the happens-before edge and lifecycle.",
    example: "Thread worker = new Thread(() -> System.out.println(\"work\"));\nworker.start();\nworker.join(); // wait for completion",
    output: "work",
    diagram: "task → thread start/happens-before → concurrent execution → join/visibility",
    benchmark: "Measure throughput, queueing, CPU, and tail latency under contention; thread count alone is not a performance target.",
    mistakes: "Starting the same Thread twice, leaking threads, busy-waiting, unsafely publishing state, and using sleep as synchronization.",
    interviewAnswer: "Thread provides a unit of concurrent execution, but manually managing threads is usually less flexible than ExecutorService. Correct concurrency requires safe publication, visibility, atomicity, and a controlled shutdown or join. Prefer immutable state and bounded executors.",
  },
  "Spring beans": {
    walkthrough: "A Spring bean is an object managed by an ApplicationContext. Component scanning or explicit @Bean methods register definitions, then the container constructs and wires instances according to scope and lifecycle rules.",
    example: "@Component\nfinal class ClockService {\n  Instant now() { return Instant.now(); }\n}\n// The ApplicationContext owns and injects ClockService.",
    output: "The container supplies one managed ClockService by default",
    diagram: "bean definition → ApplicationContext → construction/injection → managed lifecycle",
    benchmark: "Measure context startup and bean creation in a representative application; narrow scanning before tuning startup.",
    mistakes: "Instantiating a bean with new, relying on component scanning accidentally, mutable singleton state, and assuming prototype scope makes dependencies thread-safe.",
    interviewAnswer: "A Spring bean is an object created and managed by the container. Its definition controls construction, dependencies, scope, and lifecycle. Constructor injection makes required collaborators visible and lets tests assemble the object without a running context.",
  },
  "Spring profiles": {
    walkthrough: "Profiles select environment-specific bean definitions and configuration. Keep profile choice outside business logic, make the active profile explicit at deployment, and ensure a safe default does not silently point production code at a test dependency.",
    example: "@Configuration\n@Profile(\"test\")\nclass TestClockConfig {\n  @Bean Clock clock() { return Clock.fixed(Instant.EPOCH, ZoneOffset.UTC); }\n}",
    output: "The fixed clock exists only when the test profile is active",
    diagram: "deployment profile → conditional configuration → selected bean graph",
    benchmark: "Verify startup and configuration validation for every supported profile; profile selection should not be discovered only after a request fails.",
    mistakes: "Overlapping profiles, secrets in profile files, activating profiles from code, and treating profiles as a substitute for feature flags.",
    interviewAnswer: "Profiles conditionally activate configuration for environments or test scenarios. Keep environment values external, validate required properties at startup, and make the active profile observable so a deployment cannot silently run with the wrong bean graph.",
  },
  "Spring MVC": {
    walkthrough: "Spring MVC maps an HTTP request through a DispatcherServlet to a controller, then delegates domain work to services. Controllers should translate transport concerns and leave validation, authorization, and transactions to the appropriate layers.",
    example: "@RestController\nfinal class OrderController {\n  @GetMapping(\"/orders/{id}\")\n  OrderView get(@PathVariable UUID id) { return service.find(id); }\n}",
    output: "An HTTP GET is translated into a service operation and response view",
    diagram: "HTTP request → DispatcherServlet → controller → service → response mapping",
    benchmark: "Measure endpoint p50/p95 latency, serialization, database calls, and dependency failures separately; controller time alone is not request cost.",
    mistakes: "Business logic in controllers, exposing entities, missing validation/error mapping, and unbounded request bodies or pagination.",
    interviewAnswer: "Spring MVC routes requests through DispatcherServlet and handler mappings to controller methods. Keep controllers thin: bind and validate input, call a service, and map the result. Central exception handling and stable response contracts keep failures consistent.",
  },
  "Spring validation": {
    javaVersions: "16+",
    walkthrough: "Validation protects boundaries before domain work. Bean Validation annotations express structural constraints, while domain services enforce rules that depend on state or other entities. Return a stable error shape and never trust client validation alone.",
    example: "record CreateUser(@NotBlank String name, @Email String email) {}\n@PostMapping(\"/users\")\nUserView create(@Valid @RequestBody CreateUser request) { return service.create(request); }",
    output: "Malformed input is rejected before the service mutates state",
    diagram: "request binding → structural validation → domain invariant → state change",
    benchmark: "Measure rejected and accepted request latency; cap payload size and collection lengths before validation work grows unbounded.",
    mistakes: "Validating only in the browser, mixing persistence checks into annotations, leaking field internals, and accepting unknown fields unintentionally.",
    interviewAnswer: "Use Bean Validation for shape and simple constraints at the transport boundary, then enforce stateful business rules in the service/domain layer. Map violations to a stable client error and keep validation deterministic and bounded.",
  },
  "Spring Boot configuration": {
    javaVersions: "16+",
    walkthrough: "Spring Boot assembles auto-configuration from the classpath and external properties. Treat configuration as a typed contract: bind it, validate required values at startup, and make precedence explicit across files, environment variables, and command-line arguments.",
    example: "@ConfigurationProperties(\"payments\")\nrecord PaymentProperties(URI endpoint, Duration timeout) {}\n// Bind and validate before the application accepts traffic.",
    output: "The properties bind to typed values. Add explicit validation to reject missing endpoints or invalid timeouts before accepting traffic.",
    diagram: "external properties → typed binding/validation → bean configuration → runtime behavior",
    benchmark: "Measure startup and configuration refresh only where supported; fail fast rather than discovering missing settings on the first request.",
    mistakes: "Secrets committed to source, stringly typed durations, ambiguous precedence, and silent defaults for safety-critical settings.",
    interviewAnswer: "Boot combines conventions and auto-configuration with externalized properties. Bind related settings into a typed configuration object, validate them at startup, keep secrets in a secret manager, and document which source wins when values overlap.",
  },
  "Class loading": {
    walkthrough: "Class loading locates bytecode, defines a Class, and initializes it when required. Parent delegation normally checks platform and application parents first. Loading, linking, and initialization are distinct phases with different failure modes.",
    example: "Class<?> type = Class.forName(\"com.example.Plugin\");\nSystem.out.println(type.getName());\n// Loading may trigger initialization depending on the API used.",
    output: "com.example.Plugin",
    diagram: "name → loader lookup → define/link → initialize → Class object",
    benchmark: "Profile startup class-loading time and class count with JFR or class-load logs; avoid classpath-wide scanning when an explicit registry works.",
    mistakes: "Confusing ClassNotFoundException with NoClassDefFoundError, leaking custom class loaders, and assuming two loaders share type identity.",
    interviewAnswer: "The JVM loads, links, and initializes classes through class loaders. Parent delegation avoids duplicate core classes, while custom loaders can isolate plugins. A class’s identity is its binary name plus defining loader, so the same name from two loaders is not the same type.",
  },
  "Java Memory Management": {
    walkthrough: "Java memory is a set of managed regions with different ownership and failure signals. Thread stacks hold call frames, the heap holds most objects, metaspace holds class metadata, and garbage collection reclaims unreachable heap objects. A rising heap is not automatically a leak: compare allocation rate, post-GC live set, pause time, native memory, and thread behavior.",
    howToThink: "Follow one object from allocation to reachability loss, then ask which region is exhausted and what evidence distinguishes pressure from retention.",
    realWorldAnalogy: "Intuition only: a shared warehouse holds reusable items, each worker has a private desk, and an inventory clerk removes items nobody owns. A leak is an item still tagged as owned even though the business forgot why it is there.",
    example: "byte[] payload = new byte[1024];\nList<byte[]> requests = new ArrayList<>();\nrequests.add(payload);\n// If requests grows without a bound, objects remain reachable and live-set grows.",
    output: "Memory is reclaimed only after objects become unreachable and a collector processes them.",
    diagram: "allocation → reachability graph → GC discovers unreachable objects → reclaimed memory; retained objects → growing live set → heap evidence → ownership/eviction fix",
    benchmark: "Capture allocation rate, post-GC live set, GC pause time, native memory, and thread count under a representative long-lived workload.",
    mistakes: "Calling every heap increase a leak, raising -Xmx without evidence, ignoring metaspace/native memory, and retaining request data in static collections or listeners.",
    interviewAnswer: "Java memory management spans thread stacks, the shared heap, metaspace, and native areas. Garbage collection reclaims unreachable objects, but leaks happen when unnecessary references keep objects reachable. Diagnose the exhausted region with GC/JFR/heap evidence, then fix ownership, lifecycle, or bounds instead of only increasing memory.",
  },
  "Heap and stack": {
    walkthrough: "The heap stores objects shared by threads and is managed by garbage collection. Each thread has a stack of frames for method calls and local variables. Stack exhaustion and heap exhaustion are different failures with different diagnostics.",
    example: "static int depth(int n) {\n  return n == 0 ? 0 : 1 + depth(n - 1);\n}\nSystem.out.println(depth(3)); // 3",
    output: "3",
    diagram: "thread call → stack frame/local state; object allocation → shared heap → GC",
    benchmark: "Correlate allocation rate, live set, GC pauses, and thread stack usage with JFR and logs; do not fix all memory symptoms by enlarging the heap.",
    mistakes: "Calling every memory issue a leak, assuming locals always live only on stack, and changing -Xss or -Xmx without measuring the failure.",
    interviewAnswer: "Each thread has a private stack of frames, while objects generally live on the shared heap and are reclaimed when unreachable. StackOverflowError usually indicates excessive call depth; OutOfMemoryError may indicate heap pressure, native memory, or another bounded area.",
  },
  "JIT compilation": {
    walkthrough: "The JVM can interpret bytecode first, profile hot code, and compile profitable paths to optimized native code. Optimizations depend on runtime evidence and may be deoptimized when assumptions become false, so cold-start and steady-state behavior differ.",
    example: "for (int i = 0; i < 10_000_000; i++) {\n  total += i;\n}\n// Warmup and profiling influence optimized steady-state code.",
    output: "A hot loop may become optimized after warmup",
    diagram: "bytecode → interpreter/profiling → JIT compiled code → possible deoptimization",
    benchmark: "Use JMH warmup and forks; report steady-state throughput, startup, allocation, and variance instead of one wall-clock run.",
    mistakes: "Benchmarking only the first iteration, assuming every method is compiled, and adding flags before measuring the workload.",
    interviewAnswer: "JIT compilation uses runtime profiles to compile hot bytecode into optimized machine code. It can inline and specialize code, then deoptimize if assumptions fail. Reliable benchmarks separate warmup from measurement and include startup when startup latency matters.",
  },
  "JDBC basics": {
    walkthrough: "JDBC is Java’s low-level database API: obtain a connection, prepare a statement, bind parameters, execute, consume results, and close resources. Production code needs pooling, timeouts, transaction ownership, and bounded result sets.",
    example: "String sql = \"select id, status from orders where customer_id = ?\";\ntry (PreparedStatement statement = connection.prepareStatement(sql)) {\n  statement.setString(1, customerId);\n  try (ResultSet rows = statement.executeQuery()) { /* map rows */ }\n}",
    output: "A parameterized query returns only the requested customer’s rows",
    diagram: "service transaction → pooled connection → prepared SQL → ResultSet → mapped data",
    benchmark: "Measure pool wait, database execution, row mapping, and result size separately; include slow queries and saturation.",
    mistakes: "String-concatenated SQL, leaked connections, unbounded selects, missing timeouts, and assuming closing a statement commits a transaction.",
    interviewAnswer: "JDBC exposes connections, prepared statements, and result sets. Use parameter binding to prevent injection, close resources deterministically, keep transaction ownership at the service boundary, and rely on a bounded connection pool with query timeouts.",
  },
  "PreparedStatement": {
    walkthrough: "PreparedStatement separates SQL structure from values. The driver parses the statement and binds typed parameters, preventing input from changing the SQL grammar and often enabling statement reuse.",
    example: "String sql = \"select id from users where email = ?\";\ntry (PreparedStatement statement = connection.prepareStatement(sql)) {\n  statement.setString(1, email);\n  try (ResultSet rows = statement.executeQuery()) { /* consume rows */ }\n}",
    output: "The email is treated as data, not executable SQL",
    diagram: "SQL template + bound value → database parser → parameterized execution",
    benchmark: "Compare prepared and ad-hoc statements with the same workload, measuring parse cost, plan reuse, latency, and database CPU.",
    mistakes: "Concatenating user input, binding the wrong JDBC type, keeping statements open, and assuming preparation replaces authorization.",
    interviewAnswer: "PreparedStatement parameterizes SQL values and prevents injection by keeping data separate from the SQL command. It also makes type binding explicit and may enable driver/database plan reuse. It does not remove the need for authorization, limits, and query timeouts.",
  },
  "Spring Security": {
    ...
    interviewAnswer: "Authentication establishes who the caller is; authorization decides what that caller may do. Spring Security filters handle the request boundary, but object ownership and tenant rules belong in the domain/service layer. Never log credentials or rely on UI checks.",
  },
  "Spring Data JPA": {
    ...
    interviewAnswer: "Spring Data JPA generates repository behavior around JPA’s persistence context. The real performance contract is the SQL and fetch plan: inspect query counts, avoid N+1 access, use projections or deliberate fetch joins, and map entities to API DTOs.",
  },
  "Actuator and observability": {
    walkthrough: "Observability connects a request to logs, metrics, and traces. Spring Boot Actuator exposes operational endpoints, but protect them, expose only what is needed, and add business-level signals rather than relying only on JVM gauges.",
    example: "management.endpoints.web.exposure.include=health,info,metrics\nmanagement.endpoint.health.probes.enabled=true\n// Restrict the management port and require authorization.",
    output: "Health and selected metrics are available without exposing every management endpoint.",
    diagram: "request/context → structured logs + metrics + trace → alert/dashboard → diagnosis",
    benchmark: "Measure telemetry overhead and cardinality; alert on symptoms such as error rate and latency, not every low-level fluctuation.",
    mistakes: "Exposing env or beans publicly, high-cardinality labels, logging secrets, and confusing liveness with dependency readiness.",
    interviewAnswer: "Actuator provides health, metrics, and operational endpoints for Spring Boot. Expose the smallest safe set, authenticate management traffic, separate liveness from readiness, and correlate application metrics with logs and traces for diagnosis.",
  },
  "Resilience patterns": {
    walkthrough: "Resilience is explicit failure behavior: deadlines, bounded retries, circuit breaking, bulkheads, and fallbacks. Each policy must respect idempotency and downstream capacity; retries without a budget amplify outages.",
    example: "Duration deadline = Duration.ofMillis(300);\n// call once within the deadline; retry only an idempotent operation\n// with bounded attempts and jittered backoff.",
    output: "A dependency call fails within a bounded budget instead of hanging indefinitely.",
    diagram: "dependency call → timeout/retry budget → circuit/bulkhead → fallback or failure",
    benchmark: "Inject latency, errors, and recovery; measure tail latency, retry volume, saturation, and time to recover.",
    mistakes: "Retrying non-idempotent writes, synchronized retry storms, unbounded queues, and fallbacks that hide data loss.",
    interviewAnswer: "Use timeouts to bound waiting, retries only for transient and safe operations, circuit breakers to stop hammering an unhealthy dependency, and bulkheads to isolate capacity. Every policy needs observability and a defined failure outcome.",
  },
  "Testing Spring applications": {
    walkthrough: "Test the smallest layer that proves the behavior. Unit-test domain decisions without a container, use focused MVC or repository slices for integration boundaries, and reserve full-context tests for wiring and cross-layer contracts.",
    example: "@WebMvcTest(OrderController.class)\nclass OrderControllerTest {\n  @MockBean OrderService service;\n  // assert status, response shape, and service interaction\n}",
    output: "The HTTP contract is tested without booting unrelated infrastructure.",
    diagram: "behavioral contract → smallest fixture → assertion → focused feedback",
    benchmark: "Track suite duration and flake rate by test layer; parallelize safely and keep external dependencies deterministic.",
    mistakes: "Testing framework internals, overusing @SpringBootTest, brittle JSON snapshots, and mocks that assert implementation instead of behavior.",
    interviewAnswer: "Use unit tests for domain logic, slice tests for MVC or persistence boundaries, and full-context tests for wiring. Assert observable contracts and failure paths, keep fixtures deterministic, and avoid making every test pay startup cost.",
  },
  "JFR diagnostics": {
    javaVersions: "11+ (OpenJDK JFR)",
    walkthrough: "Java Flight Recorder samples low-overhead runtime events such as CPU, allocation, locks, I/O, and GC. Start a bounded recording around a reproducible workload, then correlate events with the user-visible symptom.",
    example: "jcmd <pid> JFR.start name=checkout settings=profile duration=60s filename=checkout.jfr\n// Inspect the recording with JDK Mission Control.",
    output: "A time-bounded recording links runtime events to the checkout workload.",
    diagram: "symptom → bounded JFR recording → event correlation → hypothesis → targeted fix",
    benchmark: "Compare recording overhead and signal quality under representative load; avoid collecting sensitive payloads in custom events.",
    mistakes: "Recording forever, treating samples as exact counts, ignoring time windows, and publishing recordings with secrets or identifiers.",
    interviewAnswer: "JFR is a low-overhead event recorder built into the JDK. Use it to correlate CPU, allocation, lock, I/O, and GC behavior with a bounded reproduction. It is evidence for a hypothesis, not a substitute for understanding the workload.",
  },
  "Memory leaks": {
    walkthrough: "A Java memory leak is unintended retention: objects remain reachable even though the application no longer needs them. Compare post-GC live-set growth, inspect dominators in a heap dump, and fix the retaining reference rather than only increasing heap size.",
    example: "static final Map<String, byte[]> cache = new HashMap<>();\n// Without eviction or bounds, every key remains strongly reachable.\n// Use a bounded cache policy appropriate to the data.",
    output: "A bounded ownership policy prevents unbounded cache retention.",
    diagram: "allocation → retaining reference → growing live set → heap evidence → ownership/eviction fix",
    benchmark: "Run a long-lived workload and track post-GC live set, allocation rate, and heap occupancy; compare before and after retention fixes.",
    mistakes: "Calling GC manually, increasing -Xmx as the only fix, retaining request data in static collections, and analyzing only total allocations.",
    interviewAnswer: "Leaks occur when unnecessary objects remain reachable, often through static maps, listeners, thread locals, or caches without eviction. Use GC logs, JFR, and heap-dump dominator analysis to find the retaining path, then fix ownership or bounds.",
  },
  "Packages and imports": {
    walkthrough: "A package gives types a namespace and participates in access control. An import is only source-level shorthand; it does not move classes or grant visibility. Keep package boundaries aligned with ownership and avoid wildcard imports in public examples.",
    example: "package com.example.orders;\n\nimport java.util.List;\n\npublic final class OrderService {\n  List<String> ids;\n}",
    output: "OrderService is addressed as com.example.orders.OrderService",
    diagram: "package declaration → fully qualified name → import shorthand → resolved type",
    benchmark: "Compile the same class with explicit imports and wildcard imports; compare readability and confirm both resolve to identical bytecode while package-private access remains unchanged.",
    mistakes: "Confusing imports with dependencies, relying on default packages, and assuming package-private access works across similarly named packages.",
    interviewAnswer: "Packages namespace types and define package-private visibility. Imports shorten names during compilation but do not affect runtime loading or permissions. A maintainable package structure reflects domain ownership and keeps internal types out of public contracts.",
  },
  "Regular expressions": {
    walkthrough: "A regular expression describes a text pattern; the Pattern compiles it and a Matcher applies it to input. Prefer simple anchored expressions for validation and bound input length to avoid pathological backtracking.",
    example: "Pattern email = Pattern.compile(\"^[^@\\\\s]+@[^@\\\\s]+$\");\nSystem.out.println(email.matcher(\"a@example.com\").matches()); // true",
    output: "true",
    diagram: "pattern → compiled automaton/backtracking → matcher → boolean or groups",
    benchmark: "Test worst-case inputs and measure matching time; use a linear parser when the grammar is complex or input is attacker-controlled.",
    mistakes: "Using find when matches is required, forgetting Java string escaping, catastrophic backtracking, and treating regex as full email or URL validation.",
    interviewAnswer: "Pattern compiles a regex and Matcher applies it. matches requires the whole region; find searches for a matching subsequence. Escape regex metacharacters through the Java string layer and constrain input to reduce denial-of-service risk.",
  },
  "Modules": {
    javaVersions: "9+",
    walkthrough: "The Java Platform Module System declares named modules, their exported packages, and required dependencies. A module-info.java makes encapsulation and readability explicit and enables reliable configuration beyond a flat class path.",
    example: "module com.example.orders {\n  requires java.sql;\n  exports com.example.orders.api;\n}",
    output: "Only the API package is exported to consumers",
    diagram: "module-info.java → requires/exports graph → resolver → encapsulated runtime image",
    benchmark: "Run the application once on the class path and once on the module path; compare startup diagnostics and verify that an unexported implementation package is rejected at compile time.",
    mistakes: "Exporting implementation packages, confusing requires with transitive exposure, and assuming every automatic module has a stable name.",
    interviewAnswer: "JPMS modules declare dependencies and exported packages. Requires controls readability; exports controls which packages are accessible to other modules. Modules improve strong encapsulation and deployment reasoning, but migration must account for reflection and unnamed-module dependencies.",
  },
  "Files and Paths": {
    javaVersions: "11+",
    walkthrough: "java.nio.file represents paths independently from file operations. Files provides convenience methods, while streams and channels support larger data. Treat paths as untrusted input, choose explicit charset and options, and close opened streams.",
    example: "Path report = Path.of(\"report.txt\");\nFiles.writeString(report, \"ready\", StandardCharsets.UTF_8);\nSystem.out.println(Files.readString(report)); // ready",
    output: "ready",
    diagram: "Path value → Files operation → filesystem result → closed resource",
    benchmark: "Read a bounded fixture with readString and a buffered stream, then compare elapsed time and peak memory as the fixture grows; keep the streaming approach for large inputs.",
    mistakes: "Concatenating untrusted paths, default charset assumptions, reading unbounded files into memory, and leaking directory streams.",
    interviewAnswer: "Path models a filesystem location and Files performs common operations. Use normalized, authorized roots for user-controlled paths, explicit encodings, bounded reads, and try-with-resources for streams. File APIs report both I/O and race conditions, so do not assume check-then-use is atomic.",
  },
  "Serialization boundaries": {
    javaVersions: "16+",
    walkthrough: "Serialization is a compatibility and trust boundary, not just object convenience. Prefer explicit JSON or protocol schemas for external data, validate versions and fields, and never deserialize untrusted Java object streams without a strict filter.",
    example: "record UserView(String id, String displayName) {}\n// Serialize an explicit DTO/schema rather than a domain entity graph.",
    output: "The wire contract exposes only the intended fields",
    diagram: "domain object → explicit DTO/schema → bytes → validated DTO",
    benchmark: "Serialize a representative payload through the explicit DTO schema and the full domain entity; compare payload size, fields exposed, and deserialization time while rejecting unknown or invalid fields.",
    mistakes: "Java native deserialization of untrusted input, exposing entities, missing versioning, and assuming private fields are security boundaries.",
    interviewAnswer: "Serialization crosses a compatibility and security boundary. Use explicit DTOs and versioned schemas, validate input, and control polymorphism. Native Java deserialization of untrusted bytes is dangerous; if it is unavoidable, apply strict filters and isolate the boundary.",
  },
  "synchronized and locks": {
    walkthrough: "Synchronization supplies mutual exclusion and happens-before visibility. synchronized is scoped to an intrinsic monitor; Lock adds timed, interruptible, and multiple-condition operations. Keep the critical section small and lock ordering consistent.",
    example: "private final Lock lock = new ReentrantLock();\nvoid increment() {\n  lock.lock();\n  try { count++; } finally { lock.unlock(); }\n}",
    output: "Every increment is mutually exclusive and visible to later lock holders",
    diagram: "acquire lock → critical invariant → release lock → happens-before",
    mistakes: "Unlocking on a different path, locking on publicly reachable objects, holding locks during I/O, and inconsistent lock order deadlocks.",
    interviewAnswer: "synchronized provides monitor mutual exclusion and visibility; java.util.concurrent.Lock offers features such as tryLock and interruptible acquisition. Always release explicit locks in finally, define ownership, and avoid blocking external calls while holding a lock.",
  },
  "Atomics and CAS": {
    walkthrough: "Atomic classes provide lock-free operations built on compare-and-set (CAS). CAS retries when another thread changes the value, so it is effective for small independent state but can spin under contention and does not make compound invariants automatically atomic.",
    example: "AtomicInteger sequence = new AtomicInteger();\nint id = sequence.incrementAndGet();\nSystem.out.println(id); // 1",
    output: "1",
    diagram: "read expected → CAS conditional write → retry on conflict → new value",
    benchmark: "Compare atomic, lock, and striped designs under contention; measure retries, throughput, and tail latency rather than single-thread speed.",
    mistakes: "Updating related fields separately, ABA assumptions, unbounded retry loops, and using atomics for a data structure that needs a lock or transaction.",
    interviewAnswer: "CAS atomically updates a location only if it still contains the expected value. AtomicInteger is useful for counters and state machines with one-word invariants. Related fields still require a combined synchronization strategy, and contention can make retries expensive.",
  },
  "CompletableFuture": {
    walkthrough: "CompletableFuture composes asynchronous stages and makes completion or failure explicit. Choose executor ownership, propagate exceptions, and combine independent work only when the downstream service and caller deadlines allow it.",
    example: "CompletableFuture<String> name = CompletableFuture.supplyAsync(() -> \"Ada\");\nString greeting = name.thenApply(value -> \"Hello \" + value).join();\nSystem.out.println(greeting); // Hello Ada",
    output: "Hello Ada",
    diagram: "async supplier → dependent stage → completion or exceptional completion",
    mistakes: "Blocking common-pool workers, losing exceptions, mixing incompatible executors, and composing unbounded fan-out without cancellation or deadlines.",
    interviewAnswer: "CompletableFuture represents a result that may complete later and supports dependent, combined, and exceptional stages. Use explicit executors for important workloads, handle failures deliberately, and propagate cancellation and deadlines rather than calling join everywhere.",
  },
  "Indexes and query plans": {
    walkthrough: "An index is an access structure that can avoid scanning every row, but it costs storage and write work. Read the database’s actual execution plan and design composite indexes around filter, join, and ordering predicates rather than intuition.",
    example: "CREATE INDEX idx_orders_customer_status\n  ON orders(customer_id, status);\nEXPLAIN SELECT id FROM orders\n  WHERE customer_id = ? AND status = ?;",
    output: "The plan can seek by customer_id and status instead of scanning all orders",
    diagram: "query predicates → candidate index → planner cost → scans/seeks → rows",
    benchmark: "Compare plans and p95 latency before and after the index under representative cardinality; include write amplification and cache state.",
    mistakes: "Indexing every column, ignoring column order, trusting estimates without fresh statistics, and selecting unused columns into a wide covering index.",
    interviewAnswer: "Indexes trade write/storage cost for faster access paths. Use EXPLAIN to verify the optimizer’s chosen plan, and order composite columns according to the workload’s predicates and selectivity. Re-measure after data volume and distribution change.",
  },
  "Assertions": {
    walkthrough: "Java assertions document internal invariants with assert expressions, but they are disabled by default and should not validate user input or enforce production security. Use exceptions for externally observable contract failures.",
    example: "int midpoint(int left, int right) {\n  assert left <= right : \"ordered bounds\";\n  return left + (right - left) / 2;\n}",
    output: "The invariant is checked only when assertions are enabled (-ea)",
    diagram: "internal invariant → assert predicate → diagnostic failure when enabled",
    mistakes: "Using assert for validation, side effects inside assertion expressions, and assuming assertions are enabled in production.",
    interviewAnswer: "Assertions check programmer assumptions and are enabled with -ea. Because they may be disabled, they cannot replace validation, authorization, or required error handling. Keep assertions side-effect free and use explicit exceptions for contract violations.",
  },
  "JUnit test design": {
    walkthrough: "A useful JUnit test names a behavior, controls its fixture, performs one focused action, and asserts an observable result. Keep tests deterministic and add a boundary or failure case that would have caught the defect.",
    example: "@Test\nvoid rejectsNegativeQuantity() {\n  assertThrows(IllegalArgumentException.class,\n      () -> order.add(-1));\n}",
    output: "IllegalArgumentException",
    diagram: "given fixture → when action → then observable assertion",
    benchmark: "Track runtime and flake rate by suite; optimize setup and isolation only after identifying the slow or nondeterministic layer.",
    mistakes: "Asserting implementation details, shared mutable fixtures, sleeps for synchronization, and tests that pass when the production behavior is broken.",
    interviewAnswer: "Good JUnit tests are focused executable specifications: deterministic setup, one meaningful behavior, and observable assertions. Cover success, boundary, and failure paths, and use parameterized tests when the same contract has multiple inputs.",
  },
  "Mocks and stubs": {
    walkthrough: "A stub supplies controlled answers; a mock additionally verifies interactions. Mock boundaries you own or cannot run cheaply, but prefer real value objects and fakes for stable domain behavior so tests do not mirror implementation details.",
    example: "when(rateClient.rate(\"USD\", \"EUR\")).thenReturn(new BigDecimal(\"0.92\"));\nMoney converted = service.convert(USD_10);\nverify(rateClient).rate(\"USD\", \"EUR\");",
    output: "The service uses the controlled rate and requests the expected boundary call",
    diagram: "system under test → test double boundary → controlled result/interaction assertion",
    mistakes: "Mocking every class, overspecified call order, mocks returning null by default, and tests that pass while the real integration contract is broken.",
    interviewAnswer: "Stubs control collaborator outputs; mocks verify interactions. Use them at external or expensive boundaries, keep assertions about behavior, and complement them with integration tests that prove the actual adapter contract.",
  },
  "Property-based testing": {
    walkthrough: "Property-based tests generate many inputs and check an invariant instead of enumerating only examples. Define generators that respect the domain, shrink failures to minimal counterexamples, and keep the property independent from the implementation.",
    example: "@Property\nvoid reversingTwiceReturnsOriginal(@ForAll List<Integer> values) {\n  assertEquals(values, reverse(reverse(values)));\n}",
    output: "The invariant holds across generated lists or produces a minimized counterexample",
    diagram: "domain generator → property execution → shrinking → minimal reproducible failure",
    mistakes: "Generators that never reach edge cases, properties that repeat the implementation, ignoring shrunk failures, and nondeterministic randomness.",
    interviewAnswer: "Property-based testing checks general laws over generated inputs. It is valuable for parsers, collections, serialization, and state transitions where examples miss combinations. A good generator covers boundaries and shrinking makes failures actionable.",
  },
  "Testcontainers": {
    walkthrough: "Testcontainers runs real services in disposable containers for integration tests. Pin image versions, wait for readiness, isolate data, and keep credentials and ports scoped to the test lifecycle.",
    example: "@Container\nstatic PostgreSQLContainer<?> postgres =\n    new PostgreSQLContainer<>(\"postgres:16-alpine\");\n// Configure the application from its mapped JDBC URL.",
    output: "The repository test runs against a real, isolated PostgreSQL instance",
    diagram: "test lifecycle → container startup/readiness → real adapter → cleanup",
    mistakes: "Using floating latest images, checking a port without readiness, leaking containers, and making unit tests depend on Docker.",
    interviewAnswer: "Testcontainers provides reproducible disposable dependencies for integration tests. Pin images, wait for service readiness, configure the system from mapped connection details, and keep these tests separate from fast unit tests.",
  },
  "SOLID principles": {
    walkthrough: "SOLID is a set of design heuristics: one reason to change, extension without fragile modification, substitutable subtypes, small client-specific interfaces, and dependencies pointing toward abstractions. Apply them to reduce change coupling, not as a checklist of extra classes.",
    example: "interface Notifier { void send(Message message); }\nfinal class OrderService {\n  private final Notifier notifier;\n  OrderService(Notifier notifier) { this.notifier = notifier; }\n}",
    output: "OrderService depends on a replaceable notification contract",
    diagram: "responsibility → focused abstraction → substitutable implementation → stable client",
    benchmark: "Change one notification policy in a concrete implementation and in the injected contract design; compare the number of production classes and tests that require edits.",
    mistakes: "Premature interfaces, giant ‘util’ classes, inheritance that violates substitutability, and abstractions with no second implementation or change seam.",
    interviewAnswer: "SOLID helps keep responsibilities cohesive and dependencies replaceable. Use it as a reasoning tool: identify what changes together, isolate stable contracts, and avoid abstractions that add indirection without reducing coupling.",
  },
  "Builder pattern": {
    walkthrough: "A builder makes construction readable when an object has many optional fields or validation rules. Build an immutable result, validate required combinations once, and do not let the builder’s mutable state escape.",
    example: "Report report = Report.builder()\n    .title(\"Daily\")\n    .limit(100)\n    .build();",
    output: "An immutable Report is constructed with named options",
    diagram: "fluent options → builder state → validation → immutable object",
    benchmark: "Construct the same object with a telescoping constructor and a builder across a large fixture set; compare call-site defects and allocation cost, keeping the builder only when readability or validation wins.",
    mistakes: "Builder reuse across threads, missing validation in build(), silently ignoring options, and using a builder for a two-argument value.",
    interviewAnswer: "The Builder pattern separates readable, validated construction from the final object. It is useful for many optional parameters and immutable objects, but adds ceremony; use constructors or factories when the state is small and obvious.",
  },
  "Factory pattern": {
    walkthrough: "A factory centralizes creation when the concrete type depends on input, configuration, or lifecycle policy. Keep selection rules in one place and return a stable abstraction; do not turn a factory into a service locator for every dependency.",
    example: "Parser parser = ParserFactory.forFormat(\"json\");\nDocument document = parser.parse(payload);",
    output: "The caller receives the parser matching the requested format",
    diagram: "creation request → selection rule → concrete product → abstraction returned",
    benchmark: "Instantiate each supported parser directly and through the factory; compare selection logic duplication, cold-start cost, and behavior when an unsupported format is requested.",
    mistakes: "Factories with hidden global state, stringly typed options, constructors that do too much, and factories that violate dependency injection.",
    interviewAnswer: "A factory encapsulates object selection and construction behind an abstraction. It is useful when the concrete product varies by input or configuration. Keep the factory deterministic and inject it or its registry rather than hiding all dependencies globally.",
  },
  "Strategy pattern": {
    walkthrough: "Strategy puts a variable algorithm behind a common interface so the context does not grow conditional branches. Select the strategy from trusted configuration and keep each implementation independently testable.",
    example: "interface Discount { Money apply(Money subtotal); }\nfinal class SeasonalDiscount implements Discount {\n  public Money apply(Money subtotal) { return subtotal.multiply(0.9); }\n}",
    output: "The context applies a replaceable discount policy",
    diagram: "context → strategy contract → selected algorithm → result",
    benchmark: "Run the same order set through conditional branching and injected strategies; compare throughput plus the effort to add a third policy without changing the context.",
    mistakes: "Strategies sharing mutable global state, selection based on display text, and interfaces too broad for the algorithms they represent.",
    interviewAnswer: "Strategy encapsulates interchangeable algorithms behind a contract. It removes conditional policy logic from the context and makes variants independently testable. Select using stable domain keys and keep strategy state explicit.",
  },
  "Observer pattern": {
    walkthrough: "Observer publishes a state change to registered listeners. A production event publisher needs ownership, ordering, error isolation, and listener lifecycle; synchronous observers can unexpectedly add latency to the publisher.",
    example: "publisher.subscribe(event -> audit.record(event));\npublisher.publish(new OrderPlaced(orderId));",
    output: "Subscribers receive the OrderPlaced event",
    diagram: "publisher state change → event → subscribers → isolated side effects",
    benchmark: "Publish a fixed event batch with synchronous listeners and queued listeners; compare publisher p95 latency, listener throughput, and failure isolation under one slow subscriber.",
    mistakes: "Listener leaks, reentrant publication, unbounded synchronous work, and assuming observers provide durable delivery.",
    interviewAnswer: "Observer decouples a publisher from listeners that react to changes. Define whether delivery is synchronous or durable, isolate listener failures, and unsubscribe lifecycle-bound listeners. Use a broker when cross-process durability is required.",
  },
  "Decorator pattern": {
    walkthrough: "Decorator wraps an object with the same interface and adds behavior without changing the wrapped implementation. Compose small concerns such as metrics or authorization, and preserve the underlying contract and error semantics.",
    example: "Payment secured = new AuditingPayment(new RetryingPayment(realPayment));\nsecured.charge(request);",
    output: "Auditing and retry behavior wrap the same Payment contract",
    diagram: "client → outer decorator → inner decorator → concrete component",
    benchmark: "Measure a payment call through the raw component and a two-decorator chain; compare added latency and verify metrics/auth behavior remains correct when the inner call fails.",
    mistakes: "Changing semantics silently, retrying non-idempotent calls, deep unreadable chains, and hiding the order-dependent behavior of decorators.",
    interviewAnswer: "Decorator adds responsibilities by wrapping an object with the same interface. It supports composable cross-cutting behavior, but order and failure semantics must be documented. Keep wrappers small and preserve the component contract.",
  },
  "Dependency inversion": {
    walkthrough: "Dependency inversion makes high-level policy depend on stable abstractions while infrastructure implements them. The application owns the port; adapters depend inward. Constructor injection makes the dependency direction visible.",
    example: "interface PaymentPort { Receipt charge(Money amount); }\nfinal class Checkout {\n  Checkout(PaymentPort payments) { /* inject port */ }\n}",
    output: "Checkout is testable without coupling to a payment SDK",
    diagram: "policy → owned port ← infrastructure adapter",
    benchmark: "Run checkout tests with the real SDK adapter and an in-memory port fake; compare test runtime and prove policy tests remain unchanged when the adapter is replaced.",
    mistakes: "Interfaces mirroring vendor APIs, abstractions that leak transport details, and service locators that hide dependencies.",
    interviewAnswer: "Dependency inversion separates policy from infrastructure: high-level code owns a focused port and adapters implement it. Inject dependencies explicitly and translate vendor failures at the adapter boundary.",
  },
  "REST resource design": {
    walkthrough: "REST APIs expose resources through stable nouns, HTTP semantics, representations, and explicit error contracts. Design idempotency, pagination, concurrency, and authorization before choosing endpoint shapes.",
    example: "GET /orders/42\nPOST /orders\nPATCH /orders/42\nDELETE /orders/42",
    output: "Clients use resource semantics rather than action-shaped URLs",
    diagram: "HTTP method + resource URI → validation/auth → domain operation → status/representation",
    mistakes: "Verbs in every URI, ambiguous status codes, unbounded list responses, leaking entities, and missing idempotency for retries.",
    interviewAnswer: "REST design uses resource-oriented URIs and HTTP methods with stable representations and status codes. Define authorization, pagination, optimistic concurrency, idempotency, and error shape as part of the contract.",
  },
  "Idempotency": {
    walkthrough: "An idempotent operation can be retried without changing the final result beyond the first successful application. For a retried write, accept an idempotency key, persist the outcome atomically, and return the same result for the same request identity.",
    example: "POST /payments\nIdempotency-Key: pay-7\n// Store pay-7 with the committed receipt before acknowledging success.",
    output: "A retry of pay-7 returns the original receipt instead of charging twice",
    diagram: "request key → atomic deduplication record → one side effect → replayed result",
    mistakes: "Deduplicating only in memory, expiring keys before clients stop retrying, hashing incomplete request identity, and claiming GET-like safety for non-idempotent writes.",
    interviewAnswer: "Idempotency makes retries safe, especially across network timeouts. Persist a client key and result within the same consistency boundary as the side effect, define retention, and reject conflicting payloads for a reused key.",
  },
  "Timeouts and retries": {
    walkthrough: "A timeout bounds waiting; a retry repeats work only when the failure is transient and the operation is safe. Propagate an overall deadline and use bounded, jittered backoff so callers do not create a retry storm.",
    example: "Duration deadline = Duration.ofMillis(300);\n// Attempt at most twice, with jitter, while the deadline remains.",
    output: "The call fails within a bounded 300 ms budget",
    diagram: "caller deadline → attempt → transient failure? → bounded backoff/retry → outcome",
    mistakes: "Per-attempt timeouts exceeding the caller budget, retrying writes without idempotency, synchronized backoff, and retrying validation errors.",
    interviewAnswer: "Set a caller-visible deadline, classify retryable failures, cap attempts, and add jittered backoff. Retries must respect idempotency and downstream capacity; otherwise they amplify an outage.",
  },
  "Circuit breakers": {
    walkthrough: "A circuit breaker stops calls to a failing dependency after a threshold, fails fast while open, and probes recovery in half-open state. Tune windows from measured errors and latency, and make fallback behavior explicit.",
    example: "CLOSED → failures exceed threshold → OPEN\nOPEN → cool-down probe → HALF_OPEN → success closes or failure reopens",
    output: "An unhealthy dependency is protected from continual calls",
    diagram: "outcomes → failure window → open fast-fail → half-open probe → closed/open",
    mistakes: "One global breaker for unrelated dependencies, opening on caller errors, hiding failures with unsafe fallbacks, and no recovery metrics.",
    interviewAnswer: "A circuit breaker protects a dependency by opening after measured failures, failing fast, and probing recovery in half-open state. Scope it to a meaningful dependency/operation, distinguish failure types, and monitor state transitions.",
  },
  "Event-driven design": {
    walkthrough: "Events describe facts that already happened and let consumers react asynchronously. Define ownership, schema evolution, delivery guarantees, ordering, deduplication, and recovery before choosing a broker.",
    example: "OrderPlaced v1 { orderId, customerId }\nProducer commits the order and outbox row; consumer handles duplicates idempotently.",
    output: "Consumers can rebuild or retry reactions without changing the original order fact",
    diagram: "local transaction → durable event → consumer offset → idempotent reaction/retry",
    mistakes: "Publishing before the database commit, assuming exactly-once end to end, breaking schemas, and using events to hide unclear ownership.",
    interviewAnswer: "Event-driven design communicates durable facts asynchronously. Use an outbox or equivalent atomic handoff, version schemas, make consumers idempotent, and define replay, ordering, lag, and dead-letter behavior.",
  },
  "Big-O reasoning": {
    walkthrough: "Big-O describes how work and memory grow with input size. Count dominant operations, state the assumptions, and distinguish worst-case from amortized cost. Constants matter in production, but asymptotic growth exposes scalability limits.",
    example: "boolean contains(int[] values, int target) {\n  for (int value : values) if (value == target) return true;\n  return false;\n}\n// O(n) time, O(1) extra space",
    output: "Linear scan: O(n) time and O(1) extra space",
    diagram: "input size n → operation count → dominant growth rate → capacity decision",
    benchmark: "Validate complexity with sizes such as 10², 10⁴, and 10⁶, while measuring allocation and constants on representative hardware.",
    mistakes: "Dropping a nested-loop term incorrectly, ignoring sorting or hashing costs, and treating Big-O as an exact latency prediction.",
    interviewAnswer: "Big-O classifies how an algorithm’s resource use grows with n. State time and auxiliary space, identify the dominant term, and clarify whether the bound is worst-case, average, or amortized. Then discuss constants and data sizes for production decisions.",
  },
  "Two pointers": {
    walkthrough: "Two pointers maintain a relationship between positions while moving monotonically through an ordered or constrained sequence. The invariant explains why discarded regions cannot contain a solution, giving linear time instead of nested scans.",
    example: "boolean hasPair(int[] sorted, int target) {\n  int left = 0, right = sorted.length - 1;\n  while (left < right) {\n    long sum = (long) sorted[left] + sorted[right];\n    if (sum == target) return true;\n    if (sum < target) left++; else right--;\n  }\n  return false;\n}",
    output: "The pair test runs in O(n) time on sorted input",
    diagram: "left/right invariant → compare sum → discard one proven region → advance",
    benchmark: "Compare against a hash-set or nested-loop baseline across sortedness, duplicates, and input sizes; include the sorting cost if input is not already sorted.",
    mistakes: "Applying it to unsorted data without sorting, moving both pointers, and mishandling empty or single-element input.",
    interviewAnswer: "Two pointers exploit ordering or a window relationship to move each pointer monotonically. For a sorted two-sum, a small sum advances left and a large sum retreats right; the invariant proves no discarded pair can work, yielding O(n) after sorting.",
  },
  "Sliding window": {
    walkthrough: "A sliding window maintains a contiguous range while expanding to include input and shrinking until a constraint is restored. The key is a monotonic validity rule; each element enters and leaves at most once.",
    example: "int longestAtMostTwoDistinct(String text) {\n  Map<Character, Integer> count = new HashMap<>();\n  int left = 0, best = 0;\n  for (int right = 0; right < text.length(); right++) {\n    count.merge(text.charAt(right), 1, Integer::sum);\n    while (count.size() > 2) {\n      char c = text.charAt(left++);\n      count.merge(c, -1, Integer::sum);\n      if (count.get(c) == 0) count.remove(c);\n    }\n    best = Math.max(best, right - left + 1);\n  }\n  return best;\n}",
    output: "O(n) time because left and right each advance at most n times",
    diagram: "expand right → constraint violated → shrink left → record valid window",
    benchmark: "Test empty input, all-unique input, and repeated values; compare map choices and allocation at realistic string sizes.",
    mistakes: "Shrinking too far, forgetting to remove zero counts, and using a window for a non-contiguous problem.",
    interviewAnswer: "Sliding window solves contiguous range problems with a maintainable validity condition. Expand the right edge, shrink the left edge while invalid, and update the answer only for valid windows. Monotonic pointer movement gives O(n) when updates are O(1).",
  },
  "Binary search": {
    walkthrough: "Binary search maintains a sorted search interval and discards half after each comparison. Define whether the answer is an exact value or a boundary, use overflow-safe midpoint arithmetic, and prove the interval shrinks.",
    example: "int firstAtLeast(int[] values, int target) {\n  int low = 0, high = values.length;\n  while (low < high) {\n    int mid = low + (high - low) / 2;\n    if (values[mid] < target) low = mid + 1; else high = mid;\n  }\n  return low;\n}",
    output: "The returned index is the first value >= target, or values.length",
    diagram: "sorted interval → midpoint predicate → discard half → boundary",
    benchmark: "Compare logarithmic search with a scan for tiny arrays and include sorting cost when the input is not already ordered.",
    mistakes: "Infinite loops from wrong bounds, overflow in (low+high)/2, and returning an arbitrary match when the first/last boundary is required.",
    interviewAnswer: "Binary search needs a monotonic predicate over an ordered domain. Maintain a precise inclusive/exclusive interval, calculate mid as low + (high-low)/2, and return the boundary your contract specifies. The time is O(log n) and space O(1) iteratively.",
  },
  "Recursion": {
    walkthrough: "Recursion solves a problem through smaller instances plus a base case. Prove progress toward the base case, account for stack depth, and memoize overlapping subproblems or switch to an explicit stack when depth can be large.",
    example: "int factorial(int n) {\n  if (n < 0 || n > 12) throw new IllegalArgumentException(\"n must be 0..12\");\n  return n <= 1 ? 1 : n * factorial(n - 1);\n}\nSystem.out.println(factorial(5)); // 120",
    output: "120",
    diagram: "problem → smaller call → base case → unwind/combine",
    benchmark: "Measure depth, repeated work, and allocation; compare iterative or memoized variants when n grows.",
    mistakes: "Missing base cases, no progress, exponential recomputation, and stack overflow on adversarial depth.",
    interviewAnswer: "A recursive solution needs a base case, a call on a smaller problem, and a proof that calls terminate. Analyze stack space separately from heap allocations; use memoization for overlapping subproblems and iteration for unbounded depth.",
  },
  "Dynamic programming": {
    walkthrough: "Dynamic programming stores answers to overlapping subproblems while preserving an optimal-substructure recurrence. Choose top-down memoization or bottom-up tabulation, define state and transition precisely, and bound the table dimensions.",
    example: "int fib(int n) {\n  if (n < 0 || n > 46) throw new IllegalArgumentException(\"n must be 0..46\");\n  int[] dp = new int[n + 1];\n  if (n > 0) dp[1] = 1;\n  for (int i = 2; i <= n; i++) dp[i] = dp[i - 1] + dp[i - 2];\n  return dp[n];\n}",
    output: "O(n) time instead of exponential recursive recomputation",
    diagram: "state definition → base cases → recurrence → memo/table → answer",
    benchmark: "Compare time, table memory, and rolling-state optimization at boundary n values; include integer overflow policy.",
    mistakes: "State missing required history, wrong iteration order, forgetting impossible states, and using an unbounded table.",
    interviewAnswer: "DP works when subproblems overlap and an optimal answer composes from smaller answers. Define state, base cases, transition, evaluation order, and complexity. Use memoization or tabulation and reduce memory only after preserving the needed dependencies.",
  },
  "Graph traversal": {
    walkthrough: "BFS explores a graph by distance layers; DFS explores a path before backtracking. Track visited nodes, define directed/undirected neighbor semantics, and use an explicit stack or queue when recursion depth is unsafe.",
    example: "Deque<Integer> queue = new ArrayDeque<>();\nSet<Integer> seen = new HashSet<>();\nqueue.add(0); seen.add(0);\nwhile (!queue.isEmpty()) {\n  int node = queue.removeFirst();\n  for (int next : graph.get(node)) if (seen.add(next)) queue.addLast(next);\n}",
    output: "Every reachable node is visited once: O(V + E)",
    diagram: "start → frontier queue/stack → visited set → neighbors → next frontier",
    benchmark: "Measure V, E, frontier size, and representation memory; test disconnected, cyclic, empty, and duplicate-edge graphs.",
    mistakes: "Marking visited too late, infinite cycles, recursion overflow, and assuming adjacency lists and matrices have the same complexity.",
    interviewAnswer: "BFS uses a queue and gives shortest edge distance in an unweighted graph; DFS uses a stack/recursion for reachability and structural analysis. Both are O(V+E) with adjacency lists when each node and edge is processed once.",
  },
  "Topological sorting": {
    walkthrough: "A topological order exists only for a directed acyclic graph. Kahn’s algorithm removes zero-indegree nodes while decrementing neighbors; if not all vertices are emitted, a cycle exists.",
    example: "Queue<String> ready = new ArrayDeque<>();\nfor (String node : nodes) if (indegree.get(node) == 0) ready.add(node);\n// Remove ready nodes and decrement each outgoing neighbor.",
    output: "A dependency order, or evidence that a cycle prevents one",
    diagram: "indegree zero → emit node → decrement dependents → newly ready nodes",
    benchmark: "Measure V, E, queue size, and deterministic tie-breaking; test duplicate edges, disconnected nodes, and cycles.",
    mistakes: "Applying it to undirected graphs, forgetting duplicate-edge indegrees, and returning a partial order as if it were valid.",
    interviewAnswer: "Topological sorting orders vertices so every directed edge points forward, which requires a DAG. Kahn’s indegree algorithm and DFS cycle detection both run in O(V+E). Emit all vertices or report a cycle; partial output is not a valid schedule.",
  },
  "Union-find": {
    walkthrough: "Disjoint-set union tracks connected components under repeated merges and queries. Path compression flattens find paths and union by rank or size keeps trees shallow, giving near-constant amortized operations.",
    example: "DisjointSet dsu = new DisjointSet(5);\ndsu.union(0, 1);\nSystem.out.println(dsu.connected(0, 1)); // true",
    output: "true",
    diagram: "element → parent representative → union by size → compressed path",
    benchmark: "Measure find/union sequences with and without compression; include component count and memory O(n).",
    mistakes: "Forgetting to update sizes, comparing non-representatives, and using DSU when deletions or path queries are required.",
    interviewAnswer: "Union-find maintains a partition under union and connectivity queries. Find returns a representative; path compression and union by size/rank yield inverse-Ackermann amortized time. It handles merges well but not arbitrary deletions or shortest paths.",
  },
  "Range queries": {
    walkthrough: "Range-query structures trade preprocessing and update cost for fast aggregate queries. Prefix sums handle static sums; Fenwick or segment trees handle updates with logarithmic query and update time.",
    example: "long[] prefix = {0, 3, 8, 10};\nlong sumFrom1To2 = prefix[3] - prefix[1];\nSystem.out.println(sumFrom1To2); // 7",
    output: "7",
    diagram: "array → preprocessing structure → range query/update → aggregate",
    benchmark: "Compare scan, prefix sum, Fenwick, and segment tree across query/update ratios and memory limits.",
    mistakes: "Off-by-one prefix indexes, using static prefix sums after updates, and choosing a tree when a simple scan is enough.",
    interviewAnswer: "Prefix sums answer static range sums in O(1) after O(n) preprocessing. Fenwick trees support point updates and prefix/range sums in O(log n) with O(n) memory; segment trees support richer associative aggregates and range updates with more structure.",
  },
  "JDK vs JRE vs JVM": {
    summary: "JDK, JRE, and JVM describe different layers of Java: the development tools, the runtime libraries, and the bytecode-execution engine. Learn the compile-and-run path and the modern packaging caveat.",
    memoryHook: "Build with the JDK; run with the Java runtime; execute bytecode on the JVM.",
    practice: "Compile and run the example, then remove the JDK from a runtime image only after verifying that the required modules and launcher are present.",
    walkthrough: "The JDK is the developer kit: it contains tools such as javac (the compiler), java (the launcher), javadoc, and diagnostics tools, along with the runtime. The JRE is the runtime concept: the standard libraries, supporting files, and a JVM needed to run already-compiled Java code. The JVM is the execution engine/specification: it loads and executes Java bytecode, manages runtime memory, and can JIT-compile hot code. In modern Java, do not assume there is a separately installed JRE: JDK 9 and 10 still offered separate JRE downloads, but JDK 11 and later do not have a JRE image, so deployments commonly use a JDK or build a trimmed runtime image.",
    howToThink: "Follow the artifact, not the acronym: source code needs the JDK compiler; a .class or JAR needs a compatible Java runtime; the JVM is the part that executes the bytecode on the host platform.",
    example: "// Greeting.java\npublic class Greeting {\n  public static void main(String[] args) {\n    System.out.println(\"Hello from the JVM\");\n  }\n}\n\n// Build with the JDK:\n// javac Greeting.java\n\n// Run with the Java launcher; it starts a JVM:\n// java Greeting",
    output: "Hello from the JVM",
    diagram: "Greeting.java\n  └─ javac (JDK tool) ─→ Greeting.class bytecode\n       └─ java launcher + runtime libraries ─→ JVM executes bytecode ─→ program output",
    benchmark: "This is not a JDK-versus-JRE performance choice. Verify the environment instead: run java -version and javac -version, then test the deployed JAR on the same major Java version and CPU architecture as production. Measure application startup, memory, and throughput separately.",
    mistakes: "Calling the JVM the compiler, treating the JRE as only the JVM, assuming Java source runs directly without a compiler, or assuming every JDK 11+ vendor distribution includes a separate JRE installer. Also remember that bytecode compatibility has direction: code compiled for a newer class-file version will not run on an older runtime.",
    productionNote: "Pin one supported JDK distribution and major version for builds and production. For containers, use a maintained runtime base image or create a minimal runtime with jlink when you understand the modules your application needs. Record java -version at startup and patch the runtime as part of normal security maintenance.",
    exercise: "Compile Greeting.java with your installed JDK, inspect the bytecode with javap -c Greeting, and run it. Then intentionally run it on an older Java runtime to observe the class-version error and explain why the runtime, not the JVM acronym, is the compatibility boundary.",
    interviewAnswer: "The JDK is everything needed to develop Java applications: the runtime plus tools such as javac, javadoc, and debuggers. The JRE is the runtime environment—standard libraries and supporting files plus a JVM—for running compiled code. The JVM is the virtual machine that executes Java bytecode and provides the runtime behavior that makes the same bytecode portable across supported operating systems and architectures. The useful flow is source → javac → .class bytecode → java launcher/JVM. One modern caveat: JDK 9 and 10 still had separate JRE downloads, but JDK 11+ does not have a JRE image, so current deployments usually use a JDK or a deliberately built runtime image.",
  },
  "HashMap": {
    ...
    example: "Map<String, String> orders = new HashMap<>();\norders.put(\"u-7\", \"paid\");\nSystem.out.println(orders.get(\"u-7\")); // paid",
    output: "paid",
...
function buildTutorialEditorialContent(title, category, level) {
  ...
  const detailByCategory = {
    Collections: {
      why: "The collection's ordering, equality, and ownership rules match the operation being performed.",
      failure: "It fails when keys mutate after insertion, ordering is assumed but not guaranteed, or shared access is unsynchronized.",
      complexity: "State the average and worst-case lookup/update cost, plus memory and ordering trade-offs.",
      edgeCases: "Empty collection, duplicate values, mutable keys, null policy, resize boundaries, and concurrent iteration.",
    },
    Concurrency: {
      why: "A named invariant is protected by ownership, a happens-before edge, or one atomic transition.",
      failure: "It fails under lost updates, stale visibility, deadlock, cancellation races, or an unbounded queue.",
      complexity: "Compare contention, blocking time, throughput, queue memory, and cancellation behavior—not only big-O.",
      edgeCases: "Zero work, interruption, timeout, executor shutdown, duplicate delivery, and maximum concurrency.",
    },
    Algorithms: {
      why: "Each state transition preserves the invariant, so discarded candidates cannot contain a valid better answer.",
      failure: "It fails when the input is not monotonic, the invariant is initialized incorrectly, or a boundary is skipped.",
      complexity: "Give time and auxiliary-space complexity, then compare it with the brute-force baseline.",
      edgeCases: "Empty input, one item, duplicates, negative values, already sorted/reversed data, and maximum constraints.",
    },
    Data: {
      why: "The access path and consistency boundary match the query shape and business invariant.",
      failure: "It fails when cardinality estimates are wrong, locks are held too long, or retries duplicate a write.",
      complexity: "Call out rows scanned, index/storage cost, lock duration, latency percentiles, and write amplification.",
      edgeCases: "Empty result, skewed/hot keys, nulls, concurrent writers, timeout, deadlock retry, and stale replicas.",
    },
    Architecture: {
      why: "Ownership, contracts, and failure boundaries make each state transition observable and recoverable.",
      failure: "It fails under dependency timeouts, duplicate messages, overload, partial rollout, or a lost region.",
      complexity: "Compare latency, throughput, availability, consistency, operational effort, and cost.",
      edgeCases: "Retries, replay, poison messages, schema changes, hot partitions, cold start, and degraded reads.",
    },
  };
  const details = detailByCategory[category]
    || (/concurr|thread|executor|lock/i.test(`${category} ${title}`) ? detailByCategory.Concurrency : null)
    || (/algorithm|search|sort|recursion|dynamic|graph|range|pointer|window/i.test(`${category} ${title}`) ? detailByCategory.Algorithms : null)
    || (/sql|jdbc|database|query|jpa|data/i.test(`${category} ${title}`) ? detailByCategory.Data : null)
    || (/architecture|design|rest|microservice|event|circuit|timeout|retry/i.test(`${category} ${title}`) ? detailByCategory.Architecture : null)
    || (/collection|array|list|map|set|queue|heap|stack|generic|record|enum|type|string|exception|file|io|testing|junit|mock|assert|spring|jvm|memory|garbage|class|jit|profile|module|package|method|constructor|inheritance|composition|polymorph|interface|access|operator|control|primitive|cast|stream|collector|date|optional|lambda|functional/i.test(`${category} ${title}`) ? {
      why: "The language or framework contract makes the intended state, ownership, and failure behavior explicit at the boundary.",
      failure: "It fails when the contract is misunderstood, lifecycle ownership is implicit, or a mutable/shared value crosses the wrong boundary.",
      complexity: "State the runtime and memory cost, then include compatibility, allocation, synchronization, and maintenance trade-offs.",
      edgeCases: "Nulls, empty values, boundary sizes, repeated use, mutation after publication, concurrency, and version compatibility.",
    } : null)
    || {
    why: "The stated contract and invariant connect the input to a predictable observable result.",
    failure: "It fails when assumptions are implicit, lifecycle ownership is unclear, or a dependency exceeds its budget.",
    complexity: "State the time/space cost and the practical latency, memory, compatibility, or maintenance trade-off.",
    edgeCases: "Empty input, nulls, boundary values, repeated calls, version mismatch, and dependency failure.",
  };
  return {
    ...family,
    realWorldAnalogy: buildRealWorldAnalogy(title, category),
    why: details.why,
    failure: details.failure,
    complexity: details.complexity,
    edgeCases: details.edgeCases,
    workedExample: `Trace ${title} with one normal input, then change exactly one assumption and name the resulting state transition.`,
...
function buildRealWorldAnalogy(title, category) {
  const lower = title.toLowerCase();
  if (lower.includes("inheritance")) return "Intuition only: a child may inherit family traits from a parent, but it is still a distinct person and must honor the rules it claims to follow. In code, a subtype can specialize a parent contract without breaking callers.";
  if (lower.includes("composition")) return "Intuition only: a car has an engine; it is not an engine. Composition models a whole that delegates to replaceable parts.";
  if (category === "Algorithms") return `Intuition only: think of ${title} as sorting or searching a shelf while keeping one fact true after every move.`;
  if (category === "Concurrency") return "Intuition only: several people sharing one whiteboard need an agreed handoff or lock; otherwise each can overwrite the other's work.";
  if (category === "Data" || lower.includes("jdbc") || lower.includes("query") || lower.includes("sql")) return "Intuition only: a librarian uses an index to find a book instead of walking every shelf, but maintaining that index costs time when books move.";
  if (category === "JVM") return "Intuition only: the JDK is the workshop, the runtime is the equipped venue, and the JVM is the machine executing finished instructions.";
  if (category === "Spring" || category === "Spring Boot") return "Intuition only: a restaurant host routes a request to the right specialist, while the service team owns the business decision and the kitchen handles storage.";
  return `Intuition only: use ${title} like a labeled workflow—identify the input, the rule that keeps it safe, and the visible result.`;
}

function buildLearnerFacingSummary(title, content) {
  const firstSentence = String(content.walkthrough || "").split(/(?<=[.!?])\s+/)[0];
  return `${title}: ${firstSentence}`;
}

const JAVA_CATALOG_SEEDS_WITH_MEMORY = [...JAVA_CATALOG_SEEDS, ["Java Memory Management", "JVM", "Senior"]];
export const JAVA_TUTORIAL_CATALOG = JAVA_CATALOG_SEEDS_WITH_MEMORY.map(([title, category, level], index) => {
  const editorial = JAVA_EDITORIAL_CHAPTERS[title] || {};
  const generated = buildTutorialEditorialContent(title, category, level);
  const content = { ...generated, ...editorial };
  // Prefer the actual chapter example and topic-specific reasoning over family boilerplate.
  if (editorial.example) content.workedExample = editorial.workedExample || `${editorial.example}\nExpected behavior: ${editorial.output || "See the walkthrough."}`;
  if (editorial.walkthrough) content.why = editorial.why || editorial.walkthrough;
  if (editorial.mistakes) content.failure = editorial.failure || editorial.mistakes;
  return {
  id: `catalog-${index + 1}`,
  title,
  category,
  level,
  summary: editorial.summary || buildLearnerFacingSummary(title, content),
  ...
  ...content,
  howToThink: editorial.howToThink || `For ${title}, state the invariant, trace the example on an edge case, and identify the evidence that would falsify your assumption.`,
  productionNote: editorial.productionNote || `Use ${title} with explicit bounds, observable failure behavior, and a compatibility test for the contract demonstrated above.`,
  exercise: editorial.exercise || `Implement the ${title} example, test an empty or adversarial input, and explain its complexity and failure behavior.`,
  author: "InterviewIQ Java Editorial Team",
  reviewedAt: "2026-08-27",
  javaVersions: JAVA_EDITORIAL_CHAPTERS[title]?.javaVersions || (category === "Modern Java" || category === "Java 21" ? "17–21" : "8+"),
  ...
  references: category === "Concurrency"
    ? [{ title: "Java Language Specification: Threads and Locks (Java SE 21)", url: "https://docs.oracle.com/javase/specs/jls/se21/html/jls-17.html" }]
    : category === "Collections"
      ? [{ title: "Java SE 21 Collections API", url: "https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/package-summary.html" }]
      : category.startsWith("Spring")
        ? [{ title: "Spring Framework reference", url: "https://docs.spring.io/spring-framework/reference/" }]
        : [{ title: "Java Language Specification (Java SE 21)", url: "https://docs.oracle.com/javase/specs/jls/se21/html/index.html" }],
  editorialStatus: editorial.walkthrough ? "curated" : "topic-aware",
  };
});

// Only individually authored chapters are suitable for the learner-facing library.
// The full catalog remains available internally as an editorial backlog.
export const JAVA_CURATED_TUTORIAL_CATALOG = JAVA_TUTORIAL_CATALOG.filter(({ editorialStatus }) => editorialStatus === "curated");
...
export const JAVA_QUIZ_BANK = JAVA_TUTORIAL_CATALOG.slice(0, 48).map((topic, index) => ({
  id: `quiz-${index + 1}`,
  topic: topic.title,
  question: `What problem does ${topic.title} solve, how does it work, and when would you avoid it?`,
  answer: `${topic.walkthrough} Why it works: ${topic.why} Avoid or revisit it when: ${topic.failure} Check: ${topic.edgeCases}`,
}));
...
export const JAVA_DIGEST_ARTICLES = [
  ...
  {
    id: "sql-indexes-query-plans",
    trackId: "data-sql",
    title: "SQL Indexes and Query Plans",
    level: "Mid-Senior",
    readMinutes: 9,
    format: "Performance",
    summary: "Choose indexes from predicates, ordering, selectivity, and write cost; validate with the actual query plan instead of intuition.",
    learn: ["Read estimated versus actual rows and identify scans, joins, sorts, and spills.", "Explain composite-index column order and the cost of write amplification.", "Measure before and after with representative parameters and p95/p99 latency."],
    questions: ["Why might an index not be used?", "How do you choose column order in a composite index?", "What evidence proves an index improved the query?"]
  },
  {
    id: "transaction-isolation-locking",
    trackId: "data-sql",
    title: "Transaction Isolation and Locking",
    level: "Senior",
    readMinutes: 10,
    format: "Deep Dive",
    summary: "Connect isolation levels to dirty reads, non-repeatable reads, phantoms, locks, deadlocks, and the business invariant being protected.",
    learn: ["Name the anomaly a workload can tolerate before selecting isolation.", "Keep transactions short and acquire locks in a consistent order.", "Test conflicting transactions, retries, and deadlock handling deterministically."],
    questions: ["Which anomaly does repeatable read prevent?", "How do you diagnose a deadlock?", "When is optimistic locking a better fit?"]
  },
  {
    id: "keyset-pagination",
    trackId: "data-sql",
    title: "Offset and Keyset Pagination",
    level: "Mid",
    readMinutes: 7,
    format: "API + SQL",
    summary: "Compare offset and cursor pagination using stable ordering, index support, consistency under inserts, and a bounded response contract.",
    learn: ["Use a unique deterministic cursor such as (created_at, id).", "Explain why large offsets scan and discard rows.", "Define cursor encoding, expiry, and behavior when a row is deleted."],
    questions: ["When is offset pagination acceptable?", "Why must cursor ordering be unique?", "How do you paginate while new rows arrive?"]
  },
  {
    id: "jdbc-transaction-boundaries",
    trackId: "data-sql",
    title: "JDBC Transactions and Resource Safety",
    level: "Mid",
    readMinutes: 8,
    format: "Java + SQL",
    summary: "Keep connections, statements, result sets, commits, rollbacks, and timeouts explicit when working below the ORM layer.",
    learn: ["Use prepared statements and try-with-resources for owned JDBC resources.", "Keep transaction scope aligned with one business operation.", "Define rollback, timeout, isolation, and retry behavior for transient failures."],
    questions: ["What must happen when a statement fails after earlier writes?", "Why can a connection pool exhaust?", "When is explicit JDBC preferable to an ORM?"]
  },
  {
    id: "preventing-overselling",
    trackId: "data-sql",
    title: "Preventing Overselling with Database Invariants",
    level: "Senior",
    readMinutes: 9,
    format: "Concurrency + SQL",
    summary: "Protect inventory or quota with conditional updates, constraints, short transactions, and observable retry behavior.",
    learn: ["Put the invariant in a conditional update or database constraint.", "Treat retries and duplicate commands as normal distributed-system behavior.", "Measure conflict, deadlock, retry, and rejected-request rates."],
    questions: ["Why is read-then-write unsafe under concurrency?", "How would you handle a deadlock retry?", "What does idempotency add to the workflow?"]
  },
  {
