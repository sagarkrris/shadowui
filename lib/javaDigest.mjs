...
export const JAVA_INTERVIEW_QA = [
  ...
  },
  {
    ...
    answer: "Spring commonly applies transaction behavior through an AOP proxy around a service method invoked through that proxy; public service methods are the most portable default. The proxy starts, commits, or rolls back according to the configured rules; the database transaction does not automatically include remote calls or non-proxied self-invocations. Put the boundary around a cohesive use case, keep it short, choose propagation and isolation deliberately, and translate persistence failures at the domain boundary.",
...
export const JAVA_PRODUCTION_GLOSSARY = [
  { term: "p50 / p95 / p99 latency", definition: "The time within which 50%, 95%, or 99% of requests finish. p99 exposes the slowest 1% of requests, where customers often notice an incident first." },
  { term: "SLO", definition: "Service-level objective: a measurable reliability target, such as 99.9% successful requests or p99 latency below 500 ms." },
  { term: "RED metrics", definition: "A service-health view of request Rate, Errors, and Duration." },
  { term: "Trace and span", definition: "A trace follows one request across services; each span is one timed operation within that request, such as a database query or HTTP call." },
  { term: "GC and heap", definition: "Garbage collection (GC) reclaims unreachable objects from the heap, the JVM memory area that holds most objects." },
  { term: "JFR", definition: "Java Flight Recorder: low-overhead JVM diagnostics for events such as allocation, GC, locks, and CPU samples." },
  { term: "OOM", definition: "Out of memory: the process cannot allocate more memory and may fail or be restarted." },
  { term: "Connection pool", definition: "A bounded set of reusable database or HTTP connections. Waiting for one is a capacity signal, not proof that more application threads will help." },
  { term: "N+1 query", definition: "One query loads N parent records, then another query runs for each parent—often an accidental performance problem." },
  { term: "DLQ", definition: "Dead-letter queue: a quarantined destination for messages that exceed their retry policy and need investigation or controlled replay." },
  { term: "TTL", definition: "Time to live: how long a cached value remains usable before it expires." },
  { term: "Idempotency", definition: "A repeated request has the same intended business effect as one request, so retries do not duplicate a charge, order, or state change." },
  { term: "Canary rollout", definition: "Release a new version to a small portion of traffic first, compare it with the current version, then expand or roll back." },
  { term: "Feature flag", definition: "A runtime switch that can enable or disable a behavior without deploying another build." },
  { term: "Kafka lag and rebalance", definition: "Lag is the backlog between produced and processed events. A rebalance redistributes partitions among consumers and temporarily pauses useful processing." },
];

export const JAVA_PRODUCTION_SCENARIOS = [
  {
    id: "prod-latency-spike",
    title: "tail latency (p99) suddenly spikes",
    area: "Observability & performance",
    prompt: "At 10:15 UTC, checkout p99 latency—the time within which 99% of requests finish—rises from 300 ms to 8 s while traffic is flat. Walk through your response.",
...
export const JAVA_EDITORIAL_CHAPTERS = {
  ...
  "Virtual threads": {
    ...
    benchmark: "Compare platform and virtual threads under blocking I/O; watch native/foreign-call pinning, JDK 21–23 monitor pinning, connection pools, CPU, and downstream saturation.",
...
export const JAVA_VERSION_TOPIC_GUIDE = [
  ...
  { version: "Java 22–25", topics: "Foreign Function & Memory, Stream Gatherers, virtual-thread monitor-pinning removal, and evolving preview APIs", focus: "StructuredTaskScope remains preview in JDK 25; verify the exact target JDK's release notes and preview flags before production use" },
];
...
export const JAVA_DIGEST_ARTICLES = [
  ...
  },
  {
    ...
    learn: [
      ...
      "Confirm build and runtime support before using preview APIs such as StructuredTaskScope: it was preview in Java 21 and remains preview in JDK 25.",
    ],
    questions: [
...
export const FRESHER_DSA_GLOSSARY = [
  { term: "DSA", definition: "Data structures and algorithms: ways to store information and the steps used to solve a problem." },
  { term: "Invariant", definition: "A fact that remains true after every step of a loop or algorithm. Example: a sliding window contains no repeated characters." },
  { term: "n", definition: "The input size, usually the number of array elements, characters, nodes, or records." },
  { term: "O(n), O(n²), O(log n)", definition: "A rough growth description for runtime or memory. O(n) grows with the input, O(n²) can do a pair of nested scans, and O(log n) repeatedly cuts the search space, as in binary search." },
  { term: "Bitmask", definition: "A number whose binary bits represent yes/no choices, useful for enumerating subsets when n is small." },
  { term: "Backtracking", definition: "Try a choice, continue, and undo that choice when the partial solution cannot work; it is controlled search, not random guessing." },
  { term: "Dynamic programming (DP)", definition: "Solve repeating smaller subproblems once, save their answers, and combine them into the final answer." },
  { term: "Hashing", definition: "Use a HashMap or HashSet to find, count, or remember values in expected constant time instead of scanning everything." },
  { term: "Two pointers / sliding window", definition: "Move one or two indexes through an array or string. A sliding window is the special case where the indexes describe a contiguous range." },
  { term: "BFS / DFS", definition: "Breadth-first search explores level by level and finds shortest paths in unweighted graphs; depth-first search follows one branch before backtracking." },
  { term: "Prefix sum", definition: "A running-total array that answers a static range-sum query by subtracting two precomputed totals." },
  { term: "Fenwick tree / segment tree", definition: "Data structures that maintain range information when values change. Fenwick trees are compact for prefix-like operations; segment trees support broader range queries." },
  { term: "Heap / priority queue", definition: "A structure that keeps the smallest or largest available item ready, without fully sorting all items." },
  { term: "Monotonic", definition: "Moving in one direction without reversing; a monotonic predicate changes from false to true only once, which enables binary search." },
  { term: "V and E", definition: "In graph complexity, V is the number of vertices (nodes) and E is the number of edges (connections)." },
];

export const FRESHER_DSA_PLAYBOOK = {
  framework: [
    "Restate the problem: identify the input, output, allowed operations, and what must be optimized. Say what counts as a valid answer and whether the caller needs any particular ordering.",
    "Start with a small example and write the expected state after each meaningful step. Include one example that exposes the likely trap, such as a duplicate, an empty input, or an unreachable node.",
    "Read the constraints before choosing an algorithm; they usually eliminate most approaches. For example, n = 20 may allow 2^n search, while n = 100,000 usually rules out an O(n^2) scan.",
    "Name the pattern and invariant in plain language before writing Java code. An invariant is the fact that must remain true after every loop iteration, such as ‘the window contains no repeated characters.’",
    "Design the Java representation: arrays for bounded numeric data, maps for lookup, queues/stacks for order, and graphs for relationships. Choose the smallest representation that makes the invariant easy to maintain, and use long when sums can overflow int.",
    "Trace normal, boundary, duplicate, empty, and maximum-size cases; then state time and memory complexity. Count the dominant work and auxiliary memory, including sorting, recursion, boxing, and any data copied by the implementation.",
    "Only after the solution works, improve constants, simplify code, and prepare likely interviewer follow-ups. Keep a correct baseline so every optimization can be compared against it with the same tests.",
  ],
  constraintMap: [
    { limit: "n <= 20", choice: "Complete search, bitmask, or backtracking", reason: "Exponential work can be acceptable when the state space is deliberately small. State the search size (for example, 2^20 is about one million subsets) and prune invalid branches early." },
    { limit: "n <= 1,000", choice: "O(n^2), O(n^2 log n), or dynamic programming", reason: "Quadratic tables and pair checks may fit depending on the time limit. Estimate the operation count rather than treating 1,000 as a guarantee." },
    { limit: "n <= 100,000", choice: "O(n), O(n log n), or graph traversal", reason: "Prefer sorting, hashing, two pointers, heaps, or linear graph routines. An O(n^2) loop would perform roughly 10 billion comparisons at this size." },
    { limit: "n >= 1,000,000", choice: "Near-linear scan with compact memory", reason: "Avoid object-heavy structures, nested loops, and recursion depth risk. Stream or batch input when the full data set does not need to be resident." },
    { limit: "Many queries or updates", choice: "Prefix sums, Fenwick tree, segment tree, or offline ordering", reason: "Precompute or maintain structure instead of rescanning each range. First classify whether values are static, point-updated, or range-updated because that determines the right structure." },
  ],
  patterns: [
