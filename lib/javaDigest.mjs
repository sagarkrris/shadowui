...
export const JAVA_INTERVIEW_QA = [
  {
    ...
    answer: "HashMap uses a table of buckets. It derives a hash from the key's hashCode, maps that hash to a bucket index, then compares keys with equals when more than one key shares a bucket. A collision is stored in a linked structure; in modern Java, a heavily-colliding bucket can be treeified. When the number of entries crosses capacity × load factor, the table resizes and entries are redistributed. Correct lookup therefore depends on a stable hashCode/equals contract and immutable key state.",
    example: "Map<UserId, Order> orders = new HashMap<>(); // UserId must have stable equals/hashCode",
    star: "Situation: a cache lookup began missing recently inserted records. Task: determine whether the cache or key was wrong. Action: inspected hashCode/equals, found a key field mutated after insertion, replaced it with an immutable value object, and added a regression test. Result: lookups became deterministic and the cache contract was documented.",
    followUps: "Why do collisions happen? What triggers resize? Why is HashMap unsafe for concurrent writes?",
  },
  {
