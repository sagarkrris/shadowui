...
function createLocalFallbackSeed(filters) {
  const state = createScenarioBankState(filters);
  const trackLabel = scenarioTrackLabel(state);
  const topic = topicLabel(state.track, state.engine, state.topic);
  const scope = `${trackLabel} ${topic}`;

  return {
    id: `local-${state.track}-${state.engine}-${state.topic}-scenario`,
    track: state.track,
    ...(state.track === "database" ? { engine: state.engine } : {}),
    topic: state.topic,
    title: `${topic} Production Scenario`,
    difficulty: state.difficulty,
    prompt: `A ${scope} change passed basic tests but is producing incorrect behavior in production. How would you identify the violated contract, make a safe fix, and prove the rollout is correct?`,
    interviewerIntent: `Tests practical ${topic} reasoning, failure isolation, and production-safe validation.`,
    answerOutline: [
      `Clarify the ${topic} contract, expected behavior, and the smallest reproducible failing case.`,
      "Inspect logs, metrics, tests, and recent changes before choosing a fix.",
      "Make the invariant explicit, add focused regression coverage, and roll out with observable success criteria.",
    ],
    deepDive: `Start by separating the symptom from the ${topic} root cause. State the contract, collect evidence from the failing path, and choose the smallest change that restores correctness. Validate with a regression test, production telemetry, and a rollback plan rather than relying on a happy-path result.`,
    traps: [
      "Applying a generic fix without naming the contract that was broken.",
      "Changing production behavior before reproducing the failure with a focused test.",
      "Declaring success without metrics, rollout checks, or rollback criteria.",
    ],
    followUps: [
      "What invariant would you add to prevent this regression?",
      "What evidence would tell you the rollout is safe?",
      "When would you choose a different design instead of patching the current one?",
    ],
    rubric: [
      "Names the relevant contract or invariant.",
      "Uses evidence and regression tests to guide the fix.",
      "Explains rollout safety and trade-offs.",
    ],
  };
}
...
function localVariantSeed(inputScenario, filters) {
  ...
  return listScenarioSeeds(filters)[0] || createLocalFallbackSeed(filters);
}
