...
export async function liveAvatarRequest(path, body, headers, fetcher = fetch) {
  ...
  // LiveAvatar currently returns 1000 for success; retain 100 for older API responses.
  if (![100, 1000].includes(payload?.code)) throw new AvatarProviderError('Avatar provider rejected request', { status: response.status, providerCode: payload?.code });
  return payload.data;
}
