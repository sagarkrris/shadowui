import { normalizeRevision } from './designRevision.mjs';
export const SYSTEM_DESIGN_GLOSSARY = [
  ["API", "A defined way for one program to request data or an action from another program."],
  ["API gateway", "The front door that routes requests and can apply authentication, rate limits, and common request rules."],
  ["Cache", "A faster, temporary copy of data. Always state how it expires and what happens when it is missing or stale."],
  ["TTL", "Time to live: how long a cached value is allowed to remain before it expires."],
  ["Queue / async work", "A holding line that lets the request finish while a worker completes slower work later."],
  ["DLQ", "Dead-letter queue: a separate holding area for messages that keep failing so they do not block healthy messages."],
  ["Idempotency", "A retry-safe rule: repeating the same request produces one intended result instead of duplicate side effects."],
  ["Outbox", "A database row written in the same transaction as business data, so a later publisher can reliably send the event."],
  ["Eventual consistency", "Different views may disagree briefly after a write, then converge; the product must explain that delay."],
  ["p95 / p99", "Tail latency: how long the slower 5% or 1% of requests take. It shows user pain hidden by an average."],
];
...
export function createSystemDesignCanvasState(value = {}) {
  ...
  return {
    ...
    sections,
    ...(input.revision ? { revision: normalizeRevision(input.revision) } : {}),
  };
}
...
export function exportSystemDesignCanvasMarkdown(value = {}) {
  ...
  if (state.revision) body.push(`## Architecture revision\n\n${JSON.stringify(state.revision, null, 2)}`);
