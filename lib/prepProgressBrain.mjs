...
export function buildDailyPrepPlanMarkdown(progress = {}) {
  const lines = [
    ...
    "",
    `Recent answer average: ${progress.readinessScore === null || progress.readinessScore === undefined ? "Not assessed" : `${progress.readinessScore}/100`}. Activity is not demonstrated mastery.`,
...
export function buildUnifiedPrepProgress({
  ...
  const questionScore = attempted
    ? ((mastered * 100) + (improving * 65) + (needsReview * 35)) / masteryMap.summary.total
    : null;
  const mockScore = mockScores.length ? mockAverage * 10 : null;
  // Completed answer evidence only. Profile fields, clicks and canvas notes are activity.
  const readinessScore = mockScores.length ? clampScore(mockAverage * 10) : null;
  const weakSpot = cleanText(weakSpots?.[0]) || (needsReview ? "due review questions" : "core interview fundamentals");
  const lanes = [
    lane("dsa", "DSA", "ti-binary-tree", questionScore, attempted ? `${attempted} practice question${attempted === 1 ? "" : "s"} assessed. ${activityCounts.dsaLab || 0} DSA activity event${activityCounts.dsaLab === 1 ? "" : "s"}.` : `${activityCounts.dsaLab || 0} DSA activity event${activityCounts.dsaLab === 1 ? "" : "s"}; complete a scored practice question to assess this lane.`, "dsaLab", "Open DSA"),
    lane("scenario", "Scenario", "ti-database-search", mockScore, mockScores.length ? `${mockScores.length} scored mock${mockScores.length === 1 ? "" : "s"} found. ${activityCounts.scenarioBank || 0} scenario activity event${activityCounts.scenarioBank === 1 ? "" : "s"}.` : `${activityCounts.scenarioBank || 0} scenario activity event${activityCounts.scenarioBank === 1 ? "" : "s"}; complete a scored mock to assess this lane.`, "scenarioBank", "Open scenarios"),
    lane("company", "Company", "ti-building", null, weakSpotCount ? `${weakSpotCount} weak spot${weakSpotCount === 1 ? "" : "s"} identified. ${activityCounts.company || 0} company activity event${activityCounts.company === 1 ? "" : "s"}. This lane has no scored company outcome yet.` : `${activityCounts.company || 0} company activity event${activityCounts.company === 1 ? "" : "s"}. Complete a scored company mock to assess this lane.`, "company", "Open company"),
    lane("java", "Java", "ti-cup", null, cleanText(profile.stack).match(/java|spring/i) ? `Java/Spring appears in your profile. ${activityCounts.javaDigest || 0} Java activity event${activityCounts.javaDigest === 1 ? "" : "s"}; profile and reading activity are not assessed mastery.` : `${activityCounts.javaDigest || 0} Java activity event${activityCounts.javaDigest === 1 ? "" : "s"}. Use a scored Java drill to assess this lane.`, "javaDigest", "Open Java"),
    lane("systemDesign", "System Design", "ti-schema", null, canvasReady ? `Canvas notes are present. ${activityCounts.canvas || 0} canvas activity event${activityCounts.canvas === 1 ? "" : "s"}; notes are not assessed design skill.` : `${activityCounts.canvas || 0} canvas activity event${activityCounts.canvas === 1 ? "" : "s"}. Capture and evaluate a design to assess this lane.`, "canvas", "Open canvas"),
  ];
  const progress = {
