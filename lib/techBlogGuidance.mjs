...
export const TECH_BLOG_GUIDANCE = {
  ...
  "spring-boot-configuration": {
    "avoid": "Avoid scattered string lookups, secret-bearing diagnostics, and live refreshes whose consistency and rollback behavior are undefined.",
    "answers": {
      "1. Treat configuration as a runtime contract": "A grouped, immutable configuration object lets the service validate related values together. It makes ownership and defaults visible and prevents one integration from starting with only some of its required settings.",
      "2. Precedence, defaults, and environment separation": "A safe default is intentional and documented; a required production value must be supplied explicitly. Profiles distinguish deployment environments, while durable business decisions belong in domain configuration or data with its own ownership.",
      "3. Validate and fail closed at startup": "Cross-field validation catches combinations that individually look valid but are unsafe together. If a security-sensitive or critical dependency configuration is invalid, readiness must not admit traffic.",
      "4. Secrets, provenance, and least exposure": "Log a non-secret provider name, configuration version, or rotation identifier—not the credential. The secret should be injected at the narrowest boundary and rotated through an owned deployment path.",
      "5. Refresh, feature flags, and change control": "A live refresh can make concurrent requests use different endpoints or policies. Restrict it to settings with atomic application, audit, validation, and rollback; restart when connection or security state must stay coherent.",
      "6. Test, deploy, and observe the effective configuration": "A process can start successfully while routing real traffic incorrectly. Test bad configurations, canary the effective version, and gate promotion on readiness plus the user-facing error and latency signals the setting affects."
    }
  }
};
