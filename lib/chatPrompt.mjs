...
- Use clear Markdown headings, concise bullets, examples in the selected language or technology, complexity, mistakes, and revision notes.`,
...
export function buildSystemPrompt(profile, options = {}) {
  ...
  const evidencePolicy = "Candidate evidence policy: Never invent experience, metrics, employers or outcomes. Resume bullets must cite a supplied evidence ID. Missing facts must remain explicit placeholders; distinguish self-reported evidence from skills to learn. Practice answers and generated rewrites are not proof of professional experience. A single answer is not broad interview readiness.";
  const evidenceContext = Array.isArray(options.evidence) ? `\nUser-supplied evidence records: ${JSON.stringify(options.evidence).slice(0, 12000)}` : "";
...
${evidencePolicy}${evidenceContext}
