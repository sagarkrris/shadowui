...
export function buildCanonicalGuideEntry(problem, archivalEntry = null) {
  ...
  const sections = (archivalEntry?.sections || []).map(section => ({ ...section, content: replacements[section.heading] || section.content }));
  for (const [heading, content] of Object.entries(replacements)) if (!sections.some(section => section.heading === heading)) sections.push({ heading, content });
