...
function makeTestCases(problem, visualizer) {
  if (problem.id === "best-time-to-buy-and-sell-stock") {
    return [
      { id: "normal", type: "normal", input: "prices = [7, 1, 5, 3, 6, 4]", expected: "5", why: "Buy at 1 and sell at 6; profit is 4 at price 5, then improves to 5 at price 6." },
      { id: "edge", type: "edge", input: "prices = [7, 6, 4, 3, 1]", expected: "0", why: "No later price exceeds the cheapest prior price, so no transaction is better than zero profit." },
      { id: "trick", type: "trick", input: "prices = [2, 2, 2]", expected: "0", why: "Equal prices do not create a profit; retain the earliest minimum and return zero." },
    ];
  }
...
function formatProblemValue(value) {
  if (Array.isArray(value)) return `[${value.map((item) => formatProblemValue(item)).join(", ")}]`;
  if (typeof value === "string") return `"${value}"`;
  return String(value);
}

function buildProblemStatement(problem, visualizer) {
  if (problem.id === "best-time-to-buy-and-sell-stock") return "Given daily stock prices, choose one earlier buy day and one later sell day to maximize profit. Track the minimum prior price and the best profit so far; return 0 when no profitable sale exists.";
  const inputLabel = problem.inputKind === "string" ? "a string input" : "an input collection";
  const patternLabel = visualizer.title.toLowerCase();
  return `Given ${inputLabel} for ${problem.title}, ${problem.summary} Find the best ${patternLabel} strategy, return the required answer, and explain why the invariant stays true while you process the input.`;
}

function buildProblemExamples(problem) {
  const [normalCase, edgeCase] = problem.testCases;
  const examples = [];

  if (normalCase) {
    examples.push({
      input: normalCase.input,
      output: normalCase.expected,
      explanation: normalCase.why,
    });
  }
  ...
  if (edgeCase) {
    examples.push({
      ...
      output: edgeCase.expected,
      explanation: edgeCase.why,
    });
  }

  if (!examples.length) {
    examples.push({
      input: `input = ${formatProblemValue(problem.defaultInput)}`,
      output: "Reason about the expected answer from the invariant.",
      explanation: `Use the ${problem.pattern} pattern to justify each state change.`,
    });
  }
...
function buildProblemConstraints(problem) {
  const constraints = [
    "Aim for the pattern-matching solution instead of a brute-force restart on every position.",
    `Keep the ${problem.pattern} invariant true after every move.`,
  ];

  if (problem.inputKind === "string") {
    constraints.push("Treat the input as immutable unless the prompt explicitly allows rebuilding it.");
  } else {
    constraints.push("Handle empty, single-item, and boundary-sized inputs without special-case bugs.");
  }

  if (problem.difficulty !== "Easy") {
    constraints.push("State the time and space trade-off before coding the optimized solution.");
  }

  return constraints;
}

function buildProblemTags(problem) {
  return [
    "LeetCode-style",
    problem.category,
    problem.pattern,
...
function makeProblem(row, index) {
  ...
  const testCases = makeTestCases({ id, title }, visualizer);
  ...
  return {
    ...
    edgeCases: id === "two-sum" ? ["duplicate values can still be valid", "negative values may be part of the pair", "no pair reaches the target"] : id === "best-time-to-buy-and-sell-stock" ? ["empty or one-day price history", "prices only fall", "equal prices produce zero profit"] : edgeCases,
    ...
    statement: buildProblemStatement(problem, visualizer),
    examples: buildProblemExamples({ ...problem, testCases }),
    constraints: buildProblemConstraints(problem),
    tags: buildProblemTags(problem),
  };
}

const BLIND75_PROBLEMS = PROBLEM_ROWS.map((row, index) => {
  const problem = makeProblem(row, index);
  const reference = javaReferences[problem.id];
  const rows = reference?.tests.replace(/^Input\s*\| Expected\s*/, "").split(/\n\s*\n|; /).filter(Boolean) || [];
  const concreteCases = rows.map((row, index) => {
    const [input, ...expected] = row.split(/\s*\|\s*| → /);
    return { id: ["normal", "edge", "trick"][index] || `case-${index}`, type: ["normal", "edge", "trick"][index] || "edge", input: input.trim(), expected: expected.join(" | ").trim().replace(/ \(.*\)$/, ""), why: `${reference.invariant} ${reference.example}` };
  }).filter(test => test.expected);
  return reference ? { ...problem, testCases: concreteCases.length ? concreteCases : problem.testCases, examples: concreteCases.map(test => ({ input: test.input, output: test.expected, explanation: test.why })), constraints: ["Non-null input; use the stated problem domain and Java int bounds unless long is used.", reference.constraints], dryRun: reference.example, statement: reference.statement, summary: reference.statement, invariant: reference.invariant, referenceExample: reference.example, referenceTests: reference.tests, referenceConstraints: reference.constraints, referenceComplexity: reference.complexity } : problem;
});
...
function buildSteps(problem) {
  ...
  if (problem.id === "best-time-to-buy-and-sell-stock") return [
    { id: "stock-minimum", title: "Track the cheapest prior price", highlight: { index: 0, minimumPrice: 7, bestProfit: 0 }, explanation: "At each price, keep the lowest buy price seen before or on today." },
    { id: "stock-profit", title: "Compare today's sale profit", highlight: { index: 2, minimumPrice: 1, bestProfit: 4 }, explanation: "At price 5, selling after buying at 1 gives profit 4." },
    { id: "stock-best", title: "Keep the best profit", highlight: { index: 4, minimumPrice: 1, bestProfit: 5 }, explanation: "At price 6, profit improves to 5; retain it as the answer." },
  ];
  const base = {
    "arrays-hashing": [
...
export function getBlind75ProblemCodeTemplate(problemOrId, stack = "") {
  ...
  const code = (language === "Java" && javaReferences[problem.id]?.code) || SPECIAL_CODE[problem.id]?.[language] || genericCode(problem, language);

  return { language, code, kind: language === "Java" && javaReferences[problem.id] ? "complete-solution" : SPECIAL_CODE[problem.id]?.[language] ? "reference-snippet" : "starter", traceKind: "illustrative" };
}
