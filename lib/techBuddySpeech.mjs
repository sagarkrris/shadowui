...
export function spokenInterviewReview(turn) {
  if (!turn) return '';
  const feedback = conciseSpeechText(turn.feedback || 'Review the detailed feedback on screen.');
  const strength = turn.strengths?.[0] ? `Strength: ${turn.strengths[0]}.` : '';
  const gap = turn.gaps?.[0] ? `Improve: ${turn.gaps[0]}.` : '';
  const followUp = turn.followUp ? `Follow-up question: ${turn.followUp}` : '';
  return [followUp, `Concise feedback: ${feedback}`, strength, gap, 'The full feedback is available on screen.'].filter(Boolean).join(' ');
}
