...
function extractScore(content) {
  const match = String(content || "").match(/(?:overall\s*)?score\s*:?\s*(\d+(?:\.\d+)?)\s*\/\s*10|\b(\d+(?:\.\d+)?)\s*\/\s*10\b/i);
...
export function deriveMockReplayTimelines(messages = [], structuredSessions = []) {
  const replays = [];
  ...
  const structuredReplays = (Array.isArray(structuredSessions) ? structuredSessions : [structuredSessions]).flatMap((session) => (session?.turns || []).filter((turn) => Number.isFinite(turn.score?.value)).map((turn, index) => {
    const score = turn.score.value;
    const question = preview(turn.question, "Interview question");
    const yourAnswer = preview(turn.answer, "Your answer");
    const gaps = turn.score.gaps?.join("; ") || "No structured gap recorded.";
    const improvedAnswer = turn.review?.nextAction || turn.score.recommendations?.[0] || "Repeat with a concrete example.";
    return { id: `structured-replay-${session.id}-${index}`, date: dateOnly(turn.createdAt), question, yourAnswer, score, gaps, idealAnswer: "Structured evaluation did not generate an ideal answer.", improvedAnswer, steps: [{ label: "Question", detail: question, tone: "neutral" }, { label: "Your answer", detail: yourAnswer, tone: "neutral" }, { label: "Score", detail: `${score}/10`, tone: score >= 8 ? "strong" : "risk" }, { label: "Gaps", detail: gaps, tone: "risk" }, { label: "Next action", detail: improvedAnswer, tone: "strong" }], actions: [{ label: "Replay mock", prompt: `Replay this question: ${question}` }], retryPrompt: `Replay this question: ${question}` };
  }));
  return [...structuredReplays, ...replays.reverse()].slice(0, 4);
}
...
export function deriveMockSessionHistory(messages = [], structuredSessions = []) {
  const sessions = [];
  ...
  const structured = (Array.isArray(structuredSessions) ? structuredSessions : [structuredSessions]).flatMap((session) => (session?.turns || []).filter((turn) => Number.isFinite(turn.score?.value)).map((turn, index) => ({ id: `structured-session-${session.id}-${index}`, title: `Mock Session ${index + 1}`, score: turn.score.value, date: dateOnly(turn.createdAt), topic: preview(turn.question, "Mock session"), answerPreview: preview(turn.answer, "Mock answer"), gapSummary: preview(turn.score.gaps?.join("; "), "Review the feedback and retry this topic."), retryPrompt: `Retry this mock session: ${turn.question}` })));
  return [...structured, ...sessions.reverse()].slice(0, 6);
}
...
function cleanAnswerText(value, fallback = "Use my latest answer from the chat.") {
  return String(value || "")
    .replace(/\s+/g, " ")
    .trim() || fallback;
}

function conciseRewrite(answer, focus) {
  const cleaned = cleanAnswerText(answer, `I addressed ${focus}.`)
    .replace(/\s+but\s+i\s+did\s+not\s+mention[\s\S]*$/i, "")
    .replace(/\s+but\s+did\s+not\s+mention[\s\S]*$/i, "")
    .replace(/^we had\s+/i, "I addressed ")
    .replace(/\sand i added\s+/i, " by adding ")
    .replace(/\sand added\s+/i, " by adding ")
    .replace(/\.$/, "");

  return preview(cleaned, `I addressed ${focus}`).replace(/\.$/, "") + ".";
}

function buildRewritePrompt({ label, role, focus, original, feedback }) {
  return [
    "Answer Rewrite Studio",
    `Version: ${label}`,
    `Target role: ${role}`,
    `Focus: ${focus}`,
    `Original answer: ${original}`,
    `Known feedback: ${feedback}`,
    "Rewrite this version without inventing facts. Ask me for any missing metric before using a specific number.",
  ].join("\n");
}

export function buildAnswerRewriteStudio({
  profile,
  messages = [],
  selectedCat = "",
  selectedSub = "",
  ...
  const original = cleanAnswerText(latestMessageContent(messages, "user"));
  const feedback = preview(extractGapSummary(latestMessageContent(messages, "assistant")), "No scored feedback captured yet.");
  const concise = conciseRewrite(original, focus);
  const metricHint = "replace [your real metric] with latency, throughput, users, cost, quality, or time saved when you have the actual number";
  const versions = [
    {
    ...
    {
      label: "Senior version",
      text: `${concise} I owned the decision path, aligned stakeholders on the risk, validated the change, and explained the operational trade-offs for ${role}.`,
    },
    {
      label: "STAR version",
      text: [
        `Situation: ${preview(original, `I was working on ${focus}`).replace(/\.$/, "")}.`,
        `Task: Stabilize the user impact while keeping the solution practical for ${role}.`,
        `Action: ${concise.replace(/\.$/, "")}, then validated the behavior and called out trade-offs.`,
        "Result: Add the real outcome metric, such as latency reduced, errors avoided, users protected, cost saved, or time recovered.",
      ].join(" "),
    },
    {
      label: "Metrics-added version",
      text: `${concise} To make the impact measurable, add the baseline, [your real metric], and the verification signal; ${metricHint}.`,
    },
    {
      label: "Interviewer-ready final answer",
      text: `${concise} Interview-ready version: I would also state the metric, the validation method, and the trade-off I accepted, then close with what I would improve next.`,
    },
  ].map((version) => ({
    ...version,
  ...
  return {
    title: "Answer Rewrite Studio",
    summary: `Six local rewrites for ${role}: concise, senior, STAR, metric-aware, and final answer polish.`,
    versions,
    actionPrompt: [
      "Answer Rewrite Studio",
      `Target role: ${role}`,
      ...
      `Original answer: ${original}`,
      `Known feedback: ${feedback}`,
      "Create a polished final answer with concise, senior, STAR, metrics-added, and interviewer-ready versions. Do not invent metrics.",
    ].join("\n"),
  };
}

const CODE_EXPLANATION_CHECKS = [
  {
    label: "Invariant",
    pattern: /\binvariant\b|always|maintain|previous|seen contains|window contains|pointer|state remains|after each step/i,
    coaching: "Name what stays true after every loop, pointer move, recursion step, or data-structure update.",
  },
  {
    label: "Edge cases",
    pattern: /edge cases?|boundary|empty|null|undefined|single|handle\s+duplicates?|duplicates?\s+values?|negative|invalid|overflow|sorted|unsorted|cycle/i,
    coaching: "Call out boundary inputs before coding: empty, one item, duplicates, invalid values, and limits.",
  },
  {
    label: "Complexity",
    pattern: /O\(|big o|time\s+is|space\s+is|time complexity|space complexity|runtime|memory/i,
    coaching: "State time and space complexity and explain what input size drives each cost.",
  },
  {
    label: "Trade-offs",
    pattern: /trade-?off|alternative|brute force|sorting|memory for|pros|cons|why choose|compared with|instead of/i,
    coaching: "Compare your chosen approach with at least one alternative and explain when the alternative wins.",
  },
];
...
function evidenceFor(text, pattern) {
  ...
  return preview(String(text).slice(index, match.index + match[0].length + 52), match[0]);
}

export function buildCodeExplanationJudge({
  profile,
  messages = [],
  selectedCat = "",
  selectedSub = "",
  ...
  const role = profile?.position || "target role";
  const focus = [selectedSub, selectedCat].filter(Boolean).join(" / ") || "current code explanation";
  const explanation = cleanAnswerText(latestMessageContent(messages, "user"), "Explain your code or dry run first.");
  const checks = CODE_EXPLANATION_CHECKS.map((check) => {
    const covered = check.pattern.test(explanation);

    return {
      label: check.label,
      covered,
      evidence: covered ? evidenceFor(explanation, check.pattern) : "Missing from the latest explanation.",
      coaching: check.coaching,
      prompt: [
        "Code Explanation Judge",
        `Target role: ${role}`,
        `Focus: ${focus}`,
        `Missing area: ${check.label}`,
        `My explanation: ${explanation}`,
        check.coaching,
        "Ask me to restate only this missing part, then grade it.",
      ].join("\n"),
    };
  });
  const coveredCount = checks.filter((check) => check.covered).length;
  const score = clamp((coveredCount / checks.length) * 100);
  const missing = checks.filter((check) => !check.covered).map((check) => check.label);
  const verdict = score >= 90
    ? "Strong and interviewer-ready."
    : score >= 65
      ? `Good base. Tighten ${missing.join(", ")}.`
      : `Needs a clearer interview explanation. Add ${missing.join(", ")}.`;

  return {
    title: "Code Explanation Judge",
    score,
    verdict,
    checks,
    nextActionPrompt: [
      ...
      "Help me restate this code explanation with invariant, edge cases, complexity, and trade-offs in an interviewer-ready way.",
    ].join("\n"),
  };
}
...
export function buildResumeBulletGenerator({
  ...
  const suggestions = fallbackGaps.map((gap, index) => {
    ...
    const hasEvidence = story.title !== "Proof story needed" && metrics.length > 0;
    const after = hasEvidence
      ? `ATS-friendly ${role} bullet: ${storyAction.replace(/\.$/, "")}; actual result: ${storyResult.replace(/\.$/, "")} (${metricText}); keywords: ${skills}.`
      : `Evidence required for ${gap}: do not claim delivery or impact yet. Build a relevant project, record the real outcome, then turn that evidence into a resume bullet.`;
...
export function deriveMistakeBank(messages = [], structuredSessions = []) {
  const matches = [];
  ...
  const structuredMatches = (Array.isArray(structuredSessions) ? structuredSessions : [structuredSessions]).flatMap((session) => (session?.turns || []).flatMap((turn) => (turn.score?.gaps || []).map((gap) => ({ id: `structured-${String(gap).toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, topic: gap, correction: turn.score.recommendations?.[0] || "Repeat the answer with explicit evidence.", evidence: gap, retryPrompt: `Practice this gap: ${gap}`, messageIndex: -1 }))));
  const structuredByTopic = new Map(structuredMatches.map((item) => [item.topic, item]));
  return [...structuredByTopic.values(), ...MISTAKE_PATTERNS
