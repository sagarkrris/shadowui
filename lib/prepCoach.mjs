...
export function deriveMockScores(messages = [], structuredSessions = []) {
  const responseScores = messages
    .filter((message) => message.role === "assistant")
    .flatMap((message) => {
      const matches = String(message.content || "").matchAll(/(?:overall\s*)?score\s*:?\s*(\d+(?:\.\d+)?)\s*\/\s*10|\b(\d+(?:\.\d+)?)\s*\/\s*10\b/gi);
      return Array.from(matches, (match) => clamp(Number(match[1] || match[2]), 0, 10));
    });
  // The visible completed evaluation is the record learners see and replay.
  // Prefer it when present so dashboard, history, and replay agree.
  if (responseScores.length) return responseScores;
  const structuredScores = (Array.isArray(structuredSessions) ? structuredSessions : [structuredSessions])
    .flatMap((session) => (session?.turns || []).map((turn) => turn?.score?.value).filter(Number.isFinite));
  if (structuredScores.length) return structuredScores;
