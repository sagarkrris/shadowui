...
export function buildWorkspaceActionDisplayText(prompt, metadata = {}) {
  ...
  if (type === "designSystemSearch") {
    return withDetail(`Design Lab Search: ${metadata?.query || "System design"}`, firstPromptLine(prompt));
  }

  const article = metadata?.article;
  if (article) {
    const label = type === "javaDigestMock" ? "Java Digest Mock" : "Java Digest Coach";
    return withDetail(`${label}: ${article.title || "Selected topic"}`, article.summary || firstPromptLine(prompt));
  }

  const refresherQuestion = metadata?.refresherQuestion;
  if (refresherQuestion) {
    return withDetail("Java Senior Refresher Score", refresherQuestion.question || firstPromptLine(prompt));
  }

  const roadmap = metadata?.roadmap;
  if (roadmap) {
    return withDetail(`Java Digest Roadmap: ${roadmap.title || "Selected roadmap"}`, roadmap.days?.join("; ") || firstPromptLine(prompt));
  }

  const csesTrack = metadata?.csesTrack;
  if (csesTrack) {
    return withDetail(`Handbook Java Study: ${csesTrack.title || "Selected chapter"}`, csesTrack.focus || firstPromptLine(prompt));
  }
