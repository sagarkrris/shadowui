...
export async function findOrCreateOAuthUser({ provider, subject, email, firstName = "", lastName = "" }) {
  const database = await db();
  const identity = await database.query("SELECT id, first_name, last_name, email, email_verified, created_at FROM interviewiq_users WHERE oauth_provider = $1 AND oauth_subject = $2", [provider, subject]);
  if (identity.rows[0]) return publicUser(identity.rows[0]);
  const existing = await database.query("SELECT id, first_name, last_name, email, email_verified, created_at FROM interviewiq_users WHERE email = $1", [String(email).trim().toLowerCase()]);
  if (existing.rows[0]) { await database.query("UPDATE interviewiq_users SET oauth_provider = $1, oauth_subject = $2, email_verified = TRUE WHERE id = $3", [provider, subject, existing.rows[0].id]); return publicUser({ ...existing.rows[0], email_verified: true }); }
  const password = hashPassword(createOpaqueToken());
  const result = await database.query("INSERT INTO interviewiq_users (id, first_name, last_name, email, password_salt, password_hash, email_verified, oauth_provider, oauth_subject) VALUES ($1, $2, $3, $4, $5, $6, TRUE, $7, $8) RETURNING id, first_name, last_name, email, email_verified, created_at", [`user-${randomUUID()}`, String(firstName).trim().slice(0, 80), String(lastName).trim().slice(0, 80), String(email).trim().toLowerCase(), password.salt, password.hash, provider, subject]);
  return publicUser(result.rows[0]);
}
