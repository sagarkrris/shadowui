...
function publicUser(row) { return row ? { id: row.id, email: row.email, emailVerified: Boolean(row.email_verified), createdAt: new Date(row.created_at).toISOString() } : null; }
...
export async function createUser({ email, password }) {
  ...
  try {
    ...
    const result = await database.query("INSERT INTO interviewiq_users (id, email, password_salt, password_hash) VALUES ($1, $2, $3, $4) RETURNING id, email, email_verified, created_at", [user.id, user.email, record.salt, record.hash]);
...
export async function authenticateUser({ email, password }) {
  const database = await db();
  const result = await database.query("SELECT id, email, password_salt, password_hash, email_verified, created_at FROM interviewiq_users WHERE email = $1", [String(email || "").trim().toLowerCase()]);
  const user = result.rows[0];
  return user && verifyPassword(password, passwordRecord(user)) ? publicUser(user) : null;
}
...
export async function getUserBySession(token) {
  ...
  const database = await db();
  const result = await database.query("SELECT id, email, email_verified, created_at FROM interviewiq_users WHERE id = $1", [session.user_id]);
  return publicUser(result.rows[0]);
}
...
export async function resetPassword(token, password) {
  ...
  try {
    await client.query("BEGIN");
    const tokenResult = await client.query("SELECT t.user_id, u.email FROM interviewiq_reset_tokens t JOIN interviewiq_users u ON u.id = t.user_id WHERE t.token_hash = $1 AND t.expires_at > NOW() FOR UPDATE", [digest(token)]);
    if (!tokenResult.rows[0]) { await client.query("ROLLBACK"); return null; }
    const user = tokenResult.rows[0]; const record = hashPassword(password);
