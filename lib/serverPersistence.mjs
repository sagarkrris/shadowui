...
function publicUser(row) { return row ? { id: row.id, firstName: row.first_name || "", lastName: row.last_name || "", photoUrl: row.photo_url || "", email: row.email, emailVerified: Boolean(row.email_verified), createdAt: new Date(row.created_at).toISOString() } : null; }
...
export async function getUserBySession(token) {
  ...
  const database = await db();
  const result = await database.query("SELECT id, first_name, last_name, photo_url, email, email_verified, created_at FROM interviewiq_users WHERE id = $1", [session.user_id]);
  return publicUser(result.rows[0]);
}
