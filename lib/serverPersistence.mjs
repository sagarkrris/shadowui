...
async function initializeSchema(database) {
  ...
  await database.query("ALTER TABLE interviewiq_users ADD COLUMN IF NOT EXISTS photo_url TEXT");
  await database.query("ALTER TABLE interviewiq_users ADD COLUMN IF NOT EXISTS oauth_provider TEXT");
  await database.query("ALTER TABLE interviewiq_users ADD COLUMN IF NOT EXISTS oauth_subject TEXT");
  await database.query("CREATE UNIQUE INDEX IF NOT EXISTS interviewiq_users_oauth_idx ON interviewiq_users(oauth_provider, oauth_subject) WHERE oauth_provider IS NOT NULL AND oauth_subject IS NOT NULL");
}
...
function publicUser(row) { return row ? { id: row.id, firstName: row.first_name || "", lastName: row.last_name || "", photoUrl: row.photo_url || "", oauthProvider: row.oauth_provider || "", email: row.email, emailVerified: Boolean(row.email_verified), createdAt: new Date(row.created_at).toISOString() } : null; }
...
export async function findOrCreateOAuthUser({ provider, subject, email, firstName = "", lastName = "", photoUrl = "" }) {
  const database = await db();
  const normalizedFirstName = String(firstName).trim().slice(0, 80);
  const normalizedLastName = String(lastName).trim().slice(0, 80);
  const normalizedPhotoUrl = String(photoUrl).trim().slice(0, 2_000);
  const identity = await database.query("SELECT id FROM interviewiq_users WHERE oauth_provider = $1 AND oauth_subject = $2", [provider, subject]);
  if (identity.rows[0]) {
    const result = await database.query("UPDATE interviewiq_users SET first_name = CASE WHEN $1 = '' THEN first_name ELSE $1 END, last_name = CASE WHEN $2 = '' THEN last_name ELSE $2 END, photo_url = NULLIF($3, ''), email_verified = TRUE WHERE id = $4 RETURNING id, first_name, last_name, photo_url, oauth_provider, email, email_verified, created_at", [normalizedFirstName, normalizedLastName, normalizedPhotoUrl, identity.rows[0].id]);
    return publicUser(result.rows[0]);
  }
  const existing = await database.query("SELECT id FROM interviewiq_users WHERE email = $1", [String(email).trim().toLowerCase()]);
  if (existing.rows[0]) { const result = await database.query("UPDATE interviewiq_users SET oauth_provider = $1, oauth_subject = $2, first_name = CASE WHEN $3 = '' THEN first_name ELSE $3 END, last_name = CASE WHEN $4 = '' THEN last_name ELSE $4 END, photo_url = NULLIF($5, ''), email_verified = TRUE WHERE id = $6 RETURNING id, first_name, last_name, photo_url, oauth_provider, email, email_verified, created_at", [provider, subject, normalizedFirstName, normalizedLastName, normalizedPhotoUrl, existing.rows[0].id]); return publicUser(result.rows[0]); }
  const password = hashPassword(createOpaqueToken());
  const result = await database.query("INSERT INTO interviewiq_users (id, first_name, last_name, photo_url, email, password_salt, password_hash, email_verified, oauth_provider, oauth_subject) VALUES ($1, $2, $3, NULLIF($4, ''), $5, $6, $7, TRUE, $8, $9) RETURNING id, first_name, last_name, photo_url, oauth_provider, email, email_verified, created_at", [`user-${randomUUID()}`, normalizedFirstName, normalizedLastName, normalizedPhotoUrl, String(email).trim().toLowerCase(), password.salt, password.hash, provider, subject]);
  return publicUser(result.rows[0]);
}
...
export async function getUserBySession(token) {
  ...
  const database = await db();
  const result = await database.query("SELECT id, first_name, last_name, photo_url, oauth_provider, email, email_verified, created_at FROM interviewiq_users WHERE id = $1", [session.user_id]);
  return publicUser(result.rows[0]);
}
...
export async function exportUserData(userId) {
  const database = await db();
  const userResult = await database.query("SELECT id, first_name, last_name, photo_url, email, email_verified, created_at FROM interviewiq_users WHERE id = $1", [userId]);
...
// Share the existing bounded pool and schema initialization with reader services.
export async function getPersistenceDatabase() { return db(); }
