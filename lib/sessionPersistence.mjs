import { normalizePractice, migrateLegacyPractice } from "./dailyPractice.mjs";
import { normalizeWorkspaceTab } from "./workspaces.mjs";
import { normalizeInterviewSession } from "./interviewSession.mjs";
...
function normalizeMessages(messages) {
  ...
  return messages
    ...
    .map((message) => ({
      role: message.role,
      ...(typeof message.id === "string" ? { id: message.id.slice(0, 100) } : {}),
      ...(typeof message.buddySessionId === "string" ? { buddySessionId: message.buddySessionId.slice(0, 100) } : {}),
      content: typeof message.content === "string" ? message.content : "",
      ...(typeof message.apiContent === "string" ? { apiContent: message.apiContent } : {}),
      ...(typeof message.attemptId === "string" ? { attemptId: message.attemptId } : {}),
      ...(message.replay ? { replay: true } : {}),
      ...(message.streaming || message.interrupted ? { interrupted: true } : {}),
    }))
...
function normalizeSessionSnapshot(value = {}) {
  ...
  return {
    ...
    codeInput: typeof value.codeInput === "string" ? value.codeInput : "",
    practice: value.practice ? normalizePractice(value.practice) : migrateLegacyPractice(value.messages, value.interviewSession),
    ...
    interviewSession: normalizeInterviewSession(value.interviewSession),
    systemDesignCanvas: createSystemDesignCanvasState(value.systemDesignCanvas),
    mockTimer: value.mockTimer && typeof value.mockTimer === "object" ? {
      ...(Number.isFinite(value.mockTimer.remaining) ? { remaining: value.mockTimer.remaining } : {}),
      endsAt: Number.isFinite(value.mockTimer.endsAt) ? value.mockTimer.endsAt : null,
      status: ["idle", "answering", "paused", "review"].includes(value.mockTimer.status) ? value.mockTimer.status : "idle",
    } : { endsAt: null, status: "idle" },
  };
}
...
export function importSessionSnapshot(value) {
  try {
    ...
    if (!parsed || !parsed.snapshot || typeof parsed.snapshot !== "object" || Array.isArray(parsed.snapshot)) return null;
    if (parsed.exportVersion !== undefined && ![1, 2].includes(parsed.exportVersion)) return null;
    if (parsed.version !== undefined && ![1, 2].includes(parsed.version)) return null;
    if (!Array.isArray(parsed.snapshot.messages)) return null;
