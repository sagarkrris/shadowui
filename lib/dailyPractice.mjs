import { normalizeTechBuddySession, transitionTechBuddy } from './techBuddy.mjs';
...
export function normalizePractice(value = {}) {
  ...
  return {
    buddy: normalizeTechBuddySession(value.buddy),
...
export function recordAttempt(state, attempt) {
  ...
  if (buddy?.phase === 'question' && attempt.buddySessionId === buddy.id && attempt.questionId === buddy.current?.id) {
    buddy = transitionTechBuddy(transitionTechBuddy(buddy, { type: 'submit', id: attempt.id, answer: String(attempt.answer || '') }), { type: 'evaluated', turn: attempt });
  }
  return normalizePractice({ ...current, buddy, attempts: [...current.attempts, { ...attempt, score: scoreValue(attempt.score) }], pending: null, active: null });
}
