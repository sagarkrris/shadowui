...
const title = "Microservices Design Patterns: Boundaries, Reliability, and Evolution";
const path = "/tech-blogs/microservices-design-patterns";

for (const width of [1366, 375]) {
  test(`public microservices course preserves diagrams and exposes guidance at ${width}px`, async ({ page }) => {
    await page.setViewportSize({ width, height: 812 });
    await page.goto(path);
    await expect(page.getByRole("heading", { name: title, exact: true })).toBeVisible();
    const diagram = page.getByLabel("5. Saga and transactional outbox diagram");
    await expect(diagram).toHaveCSS("white-space", "pre-wrap");
    await expect(diagram).toContainText("Broker");
    expect(await diagram.textContent()).toContain("\n");
    await expect(page.getByRole("heading", { name: "Capstone: Split and operate an order platform" })).toBeVisible();
    const answer = page.locator("details").filter({ has: page.locator("summary", { hasText: /^Sample answer$/ }) }).first();
    await answer.locator("summary").click();
    await expect(answer.locator("p")).toContainText("business decision");
    await expect(page.getByText("When to avoid:", { exact: true })).toHaveCount(7);
  });
}

test("workspace reader exposes microservices guidance and answers", async ({ page }) => {
