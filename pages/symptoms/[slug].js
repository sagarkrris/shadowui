...
export default function SymptomEntry({ entry, source, lessonTitle, scenarioTitle }) {
  ...
  return <ReaderLayout title={entry.title} description={entry.summary}><div className={lab.entry}>
    ...
    {entry.slug === "transaction-does-not-roll-back" && <p><Link href="/learn/spring-transactions">Follow the complete Spring transaction journey →</Link></p>}<nav aria-label="Entry sections" className={styles.actions}><a href="#overview">2-minute overview</a><a href="#causes">Compare causes</a><a href="#reproduce">Try it yourself</a></nav>
