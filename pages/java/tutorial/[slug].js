...
import ReadableFlowDiagram from "../../../components/learning/ReadableFlowDiagram";
import AnswerAtAGlance from "../../../components/learning/AnswerAtAGlance";
...
export default function JavaTutorialArticle({ tutorial, previous, next }) {
  ...
  const sections = ["Answer at a glance", "Walkthrough", "How to think", "Example", "Expected result", "Diagram", "Benchmark", "Common mistakes", "Production note", "Real-world analogy", "Retention exercise", "Interview answer", "Related"];
  return <>
    ...
    <main className="tutorial-article-page" style={{ background: "#08111f", color: "#e5edf8", padding: "32px 18px" }}><article style={{ margin: "0 auto", maxWidth: 920 }}>
      ...
      <section style={{ display: "grid", gap: 16, fontSize: 16, lineHeight: 1.65 }}><section id="answer-at-a-glance"><AnswerAtAGlance category={tutorial.category} takeaway={tutorial.walkthrough} example={tutorial.workedExample || tutorial.example} why={tutorial.why} failure={tutorial.failure} complexity={tutorial.complexity || tutorial.benchmark} edgeCases={tutorial.edgeCases} /></section><Block id="walkthrough" title="Walkthrough">{tutorial.walkthrough}</Block><Block id="how-to-think" title="How to think">{tutorial.howToThink}</Block><Block id="example" title="Example"><p>{tutorial.exampleKind || "Contextual snippet: supply the imports, enclosing method and application types shown. Expected result describes the behavior; it is not necessarily console output."}</p><pre style={codeStyle}>{tutorial.example}</pre></Block>{tutorial.output && <Block id="expected-result" title="Expected result">{tutorial.output}</Block>}<Block id="diagram" title="Diagram" diagramValue={tutorial.diagram}><pre style={codeStyle}>{tutorial.diagram}</pre></Block><Block id="benchmark" title="Benchmark">{tutorial.benchmark}</Block><Block id="common-mistakes" title="Common mistakes">{tutorial.mistakes}</Block><Block id="production-note" title="Production note">{tutorial.productionNote}</Block>{tutorial.realWorldAnalogy && <Block id="real-world-analogy" title="Real-world analogy (intuition, not proof)">{tutorial.realWorldAnalogy}</Block>}<Block id="retention-exercise" title="Retention exercise">{tutorial.exercise}</Block><Block id="interview-answer" title="Interview answer">{tutorial.interviewAnswer}</Block>{tutorial.relatedTopics?.length > 0 && <Block id="related" title="Related tutorials"><div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{tutorial.relatedTopics.map((topic) => <Link key={topic} href={`/java/tutorial/${slugifyJavaTutorial(topic)}`} style={{ color: "#8bd3ff" }}>{topic}</Link>)}</div></Block>}</section>
      <p><Link href={{ pathname: "/practice", query: { buddy: "1", buddyTopic: tutorial.title } }} style={{ color: "#8bd3ff", fontWeight: 700 }}>Practice this topic with Tech Buddy →</Link></p>
      <section aria-label="Authoritative references"><h2>References and version context</h2>{tutorial.references?.map(ref => <p key={ref.url}><a href={ref.url} target="_blank" rel="noreferrer">{ref.title}</a></p>)}</section>
...
function Block({ id, title, children, diagramValue }) { return <section id={id}><h2 style={{ color: "#8bd3ff", fontSize: 24, marginBottom: 6 }}>{title}</h2><div style={{ color: "#c7d5e8" }}>{id === "diagram" ? <><ReadableFlowDiagram value={diagramValue} title="See the concept flow" /><pre style={{ ...codeStyle, marginTop: 10 }}>{children}</pre></> : children}</div></section>; }
