...
function Block({ id, title, children }) { return <section id={id}><h2 style={{ color: "#8bd3ff", fontSize: 24, marginBottom: 6 }}>{title}</h2><div style={{ color: "#c7d5e8" }}>{id === "diagram" ? <><ReadableFlowDiagram value={children} title="See the concept flow" /><pre style={{ ...codeStyle, marginTop: 10 }}>{children}</pre></> : children}</div></section>; }
